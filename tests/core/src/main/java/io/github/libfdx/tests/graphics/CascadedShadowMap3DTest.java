 ...
 * @author xpenatan
 */
public final class CascadedShadowMap3DTest extends ApplicationAdapter {
     ...
     */
    public CascadedShadowMap3DTest(long exitAfterFrames) {
    ...
    public void create(Fdx fdx) {
        ...
        sun = new DirectionalLight()
                ...
                .intensity(2.18f);
        biasState = Ui.state(floatProperty("libfdx.test.shadowBias", 0.34f));
        ...
        cameraController = new FreeCameraController3D(fdx.input(), camera)
                ...
                .pointerRegion((x, y) -> x < splitViewportX())
        ...
        created = true;
        logger.info("CascadedShadowMap3DTest created CSM reference-style scene with "
                + ("cascaded directional shadows") + " for provider "
                + graphics.providerId());
    }
    ...
    public void render() {
        ...
        editorCamera.viewport(rightWidth, height);
        cascadedShadowMap.update(camera);
        if (uiVisible) {
    ...
    public void dispose() {
        ...
        if (!created) {
            throw new FdxException("CascadedShadowMap3DTest did not create graphics resources");
        }
        if (exitAfterFrames > 0L && renderedFrames < exitAfterFrames) {
            throw new FdxException("CascadedShadowMap3DTest rendered " + renderedFrames + " of "
                    + exitAfterFrames + " required frames");
        }
        logger.info("CascadedShadowMap3DTest rendered " + renderedFrames + " frames");
    }
    ...
    private void renderShadowPreview() {
        Texture texture = cascadedShadowMap.activeShadowMap().texture();
        float x = 0.57f;
        float y = 0.48f;
        float size = 0.38f;
    ...
    private int splitViewportX() {
        return Math.max(1, framebufferWidth() / 2);
    }
    ...
    private void captureFrame(String path) {
        try {
            ...
            logger.info("CascadedShadowMap3DTest captured framebuffer to " + path);
        } catch (Exception e) {
            throw new FdxException("Could not capture CascadedShadowMap3DTest framebuffer", e);
        }
    }
