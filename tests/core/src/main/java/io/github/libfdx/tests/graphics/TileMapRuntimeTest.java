...
/**
 * Runs the tile-map rendering test scenario.
 *
 * @author xpenatan
 */
public final class TileMapRuntimeTest extends ApplicationAdapter {
    ...
    /**
     * Creates a tile-map runtime test.
     *
     * @param exitAfterFrames the exit after frames
     */
    public TileMapRuntimeTest(long exitAfterFrames) {
    ...
    public void create(Fdx fdx) {
        ...
        logger = fdx.logger();
        fpsLogger = TestFpsLogger.create(logger, "TileMapRuntimeTest");
        ...
        created = true;
        logger.info("TileMapRuntimeTest created " + MAP_WIDTH + "x" + MAP_HEIGHT
                + " map with " + tileSet.size() + " generated tiles for provider "
    ...
    public void render() {
        ...
        batch.begin(LoadOp.clear(0.05f, 0.06f, 0.08f, 1.0f));
        renderer.render(map, tileSet, batch, mapX, mapY, mapX, mapY, map.worldWidth(), map.worldHeight());
        batch.end();
    ...
    public void dispose() {
        ...
        if (!created) {
            throw new FdxException("TileMapRuntimeTest did not create graphics resources");
        }
        if (exitAfterFrames > 0L && renderedFrames < exitAfterFrames) {
            throw new FdxException("TileMapRuntimeTest rendered " + renderedFrames + " of "
                    + exitAfterFrames + " required frames");
        }
        if (capturePath != null && capturePath.length() > 0 && !captured) {
            throw new FdxException("TileMapRuntimeTest did not capture framebuffer to " + capturePath);
        }
        logger.info("TileMapRuntimeTest rendered " + renderedFrames + " frames");
    }

    private Texture createAtlas() {
        int width = TILE_SIZE * TILE_COLUMNS;
        int height = TILE_SIZE;
        Texture texture = graphics.device().createTexture(TextureDescriptor.rgba8("tile-map atlas", width, height));
    ...
    private ByteBuffer atlasPixels(int width, int height) {
        ...
        for (int y = 0; y < height; y++) {
            for (int x = 0; x < width; x++) {
                ...
                boolean line = x % TILE_SIZE == 0 || y % TILE_SIZE == 0
                        || x % TILE_SIZE == TILE_SIZE - 1 || y % TILE_SIZE == TILE_SIZE - 1;
                putRgba(pixels, line ? brighten(color) : color);
            }
        }
        pixels.flip();
        return pixels;
    ...
    private TileMap createMap() {
        ...
        for (int x = 1; x < MAP_WIDTH - 1; x++) {
            path.tile(x, MAP_HEIGHT / 2, 3);
        }
        for (int y = 1; y < MAP_HEIGHT - 1; y++) {
            path.tile(MAP_WIDTH / 2, y, 2);
        }
        return newMap;
    }
    ...
    private void captureFrame(String path) {
        try {
            ...
            logger.info("TileMapRuntimeTest captured framebuffer to " + path);
        } catch (Exception e) {
            throw new FdxException("Could not capture TileMapRuntimeTest framebuffer", e);
        }
    }
    ...
    private int brighten(int rgba) {
        int red = Math.min(255, ((rgba >>> 24) & 0xFF) + 38);
        int green = Math.min(255, ((rgba >>> 16) & 0xFF) + 38);
        int blue = Math.min(255, ((rgba >>> 8) & 0xFF) + 38);
