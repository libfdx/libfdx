...
public final class TestChooserApplication extends ApplicationAdapter {
    ...
    private static final float COPY_MENU_WIDTH = 160.0f;
    ...
    private final InputAdapter returnInputProcessor = new InputAdapter() {
        ...
        public boolean pointerDown(PointerEvent event) {
            if (currentTest != null || root == null) {
                return false;
            }
            float x = root.uiX(event.x());
            float y = root.uiY(event.y());
            if (copyTestName != null) {
                if (event.button() == MouseButton.LEFT && copyMenuItem != null
                        && copyMenuItem.bounds().contains(x, y)) {
                    return false;
                }
                closeCopyMenu();
            ...
            UiNode row = findTestRow(root.rootNode(), x, y);
            if (row != null) {
                testList.resetClick();
                copyTestName = row.modifier().semanticLabel();
                copyMenuX = Math.max(0.0f, Math.min(x, root.rootNode().bounds().right() - COPY_MENU_WIDTH));
                copyMenuY = Math.max(0.0f, Math.min(y, root.rootNode().bounds().bottom() - TEST_ROW_HEIGHT));
                root.requestCompose();
            }
            return true;
        }
    };
    private final InputRouter selectorInput = new InputRouter().add(returnInputProcessor);
    private String copyTestName;
    private float copyMenuX;
    private float copyMenuY;
    private UiNode copyMenuItem;
    private Fdx fdx;
    ...
    private boolean descriptionsStacked;
    private UiNode selectedTestRow;
    ...
    public void resize(int width, int height) {
        ...
        if (root != null) {
            ...
            if (descriptionsStacked != stackTestDescription()) {
                root.requestCompose();
            }
        }
    }
    ...
    public void dispose() {
        disposeCurrentTest();
        if (fdx != null) {
            fdx.input().removeProcessor(selectorInput);
        }
        if (root != null) {
            root.dispose();
            root = null;
    ...
    private void buildUi(UiScope ui) {
        ...
        buildMenu(ui);
        if (copyTestName != null) {
            ui.popup(Ui.popup("copy-test-name").blockingInput(true), popup -> {
                copyMenuItem = popup.button("Copy name", Ui.modifier()
                        .width(COPY_MENU_WIDTH).height(TEST_ROW_HEIGHT).offset(copyMenuX, copyMenuY), () -> {
                            fdx.input().clipboard().setText(copyTestName);
                            closeCopyMenu();
                        });
            });
        }
    ...
    private void closeCopyMenu() {
        copyTestName = null;
        copyMenuItem = null;
        root.requestCompose();
    }

    private static UiNode findTestRow(UiNode node, float x, float y) {
        if (!node.visible() || !node.bounds().contains(x, y)) {
            return null;
        }
        String style = node.modifier().style();
        if ("test-row-button".equals(style) || "test-row-selected".equals(style)) {
            return node;
        }
        for (int i = 0; i < node.children().size(); i++) {
            UiNode row = findTestRow(node.children().get(i), x, y);
            if (row != null) {
                return row;
            }
        }
    ...
    private void buildMenu(UiScope ui) {
        descriptionsStacked = stackTestDescription();
        float pagePadding = compactLayout ? 8.0f : 14.0f;
        float pageGap = compactLayout ? 6.0f : 10.0f;
        if (testList.filter(search.get())) {
            testScroll.scrollTo(0.0f, 0.0f);
        ...
        ui.column(Ui.modifier().fill().padding(pagePadding).gap(pageGap), page -> {
            ...
            page.scroll(Ui.modifier().fill().weight(1.0f), testScroll, list -> {
                TestSelector.TestDescriptor[] descriptors = testList.visible();
                if (descriptors.length == 0) {
                    list.text("No tests match your search. Try another name or clear the filter.",
                            Ui.modifier().fillWidth().padding(14.0f).style("muted"));
                }
                for (int i = 0; i < descriptors.length; i++) {
                    final TestSelector.TestDescriptor descriptor = descriptors[i];
                    buildTestRow(list, descriptor, testIndex(descriptor.name()));
                }
            });
        });
    }
    ...
    private void buildMenuHeader(UiScope page) {
        ...
        page.panel(Ui.modifier().fillWidth().padding(padding).gap(gap).style("panel-strong"), header -> {
            if (compactLayout) {
                ...
                header.text(status, Ui.modifier().fillWidth().style("muted"));
            } else {
                header.text("libfdx tests", Ui.modifier().fillWidth().style("title"));
                header.text(status, Ui.modifier().fillWidth().style("muted"));
            ...
            header.row(Ui.modifier().fillWidth().gap(8.0f), row -> {
                row.text("Search", Ui.modifier().width(64.0f).style("muted"));
                row.textField(Ui.modifier().fillWidth().weight(1.0f).height(36.0f)
                        .semanticLabel("Search tests by name or description"), search);
                row.button("Clear", Ui.modifier().width(76.0f).height(36.0f)
                        .enabled(!search.get().isEmpty()), () -> search.set(""));
            });
            header.text(testList.visible().length + " / " + testList.totalCount()
                    + " tests  |  A-Z  |  Double-click or double-tap to open",
                    Ui.modifier().fillWidth().style("muted"));
        });
    }
    ...
    private void buildTestRow(UiScope list, TestSelector.TestDescriptor descriptor, int index) {
        ...
        UiNode testRow = list.stack(Ui.modifier().fillWidth().height(testRowHeight()).margin(TEST_ROW_MARGIN), item -> {
            // Keep the whole row clickable and its clipboard identity independent of presentation text.
            item.button("", Ui.modifier().fill().style(style).semanticLabel(descriptor.name()), () -> {
                    selectedTestIndex.set(index);
                    rememberSelectedTest(descriptor.name());
                    if (testList.click(descriptor.name(), System.nanoTime())) {
                        requestLaunch(descriptor.name());
                    }
                });
            if (stackTestDescription()) {
                // The font leaves extra space below its glyphs; balance visible text, not just line boxes.
                item.column(Ui.modifier().fill().padding(14.0f, 7.0f, 14.0f, 3.0f).gap(2.0f), labels -> {
                    labels.text(descriptor.displayName(), Ui.modifier().fillWidth().style("test-name"));
                    labels.text(descriptor.description(), Ui.modifier().fillWidth().style("test-description"));
                });
            } else {
                item.row(Ui.modifier().fill().padding(14.0f, 7.0f).gap(20.0f), labels -> {
                    labels.text(descriptor.displayName(), Ui.modifier().width(320.0f).style("test-name"));
                    labels.text(descriptor.description(), Ui.modifier().fillWidth().weight(1.0f)
                            .style("test-description"));
                });
            }
        });
        if (selected) {
            selectedTestRow = testRow;
        }
    }

    private boolean stackTestDescription() {
        return compactLayout || root.uiX(display.width()) < 900.0f;
    }

    private float testRowHeight() {
        // Match the theme's line heights plus the gap and total vertical padding.
        return stackTestDescription() ? (compactLayout ? 45.0f : 51.0f) : TEST_ROW_HEIGHT;
    }

    private void buildGraphicsSelector(UiScope header) {
        if (graphicsOptions.length <= 1) {
            header.text("graphics: " + graphicsDisplayName(graphicsOptions[0]),
                    Ui.modifier().fillWidth().style("muted"));
            return;
        }
        header.row(Ui.modifier().fillWidth().gap(8.0f), row -> {
    ...
    private void scrollToSelectedTestIfNeeded() {
        if (scrollToSelectedTestPending) {
            scrollToSelectedTestPending = false;
            root.ensureVisible(selectedTestRow);
        }
    }
    ...
    private static int initialTestIndex() {
        ...
        int defaultIndex = testIndex(TestSelector.defaultTestName());
    ...
    private static UiTheme theme(boolean compact) {
        ...
        return Ui.darkTheme()
                ...
                .style("test-name", UiStyle.style().text(listButtonText))
                .style("test-description", UiStyle.style().text(muted.wrap(false).ellipsis(true)))
