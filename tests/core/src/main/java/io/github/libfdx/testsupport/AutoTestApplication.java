...
public final class AutoTestApplication extends ApplicationAdapter {
    ...
    private static final float DEFAULT_TEST_DURATION_SECONDS = 6.0f;
    ...
    private void nextTest() {
        ...
        while (currentIndex < tests.length
                && (!tests[currentIndex].supports(graphics.device().capabilities())
                || (tests[currentIndex].requiresAudio() && fdx.audio() == null))) {
            TestSelector.TestDescriptor skipped = tests[currentIndex];
            skippedTests++;
            completedTests++;
            System.out.println("[info] Auto test " + (currentIndex + 1) + "/" + tests.length + ": "
                    + skipped.name() + " skipped; "
                    + (skipped.requiresAudio() && fdx.audio() == null ? "platform has no audio provider"
                    : "provider " + graphics.providerId().value() + " does not support "
                    + skipped.unsupportedFeatures(graphics.device().capabilities())));
            currentIndex++;
        }
    ...
    private void printSummary() {
        ...
        if (completed) {
            System.out.println("[info] Auto test runner complete: " + passed + " passed, "
                    + skippedTests + " skipped, " + failed + " failed. Visual and interactive checks still require review.");
        } else {
    ...
    private void rememberFailure(Throwable error) {
        ...
        // Every failure is logged and summarized separately. Avoid suppressed-exception
        // mutation here: a secondary cleanup error must not crash portable reporting.
    }
    ...
    private void buildOverlay(UiScope ui) {
        ui.column(Ui.modifier().fill().padding(12.0f).gap(8.0f), page -> {
            page.panel(Ui.modifier().fillWidth().padding(10.0f).gap(4.0f).style("overlay-panel"), panel -> {
                panel.text(primaryStatus(), Ui.modifier().fillWidth().minHeight(52.0f)
                        .style(currentTestFailed ? "error" : "title"));
                panel.text(secondaryStatus(), Ui.modifier().fillWidth().minHeight(40.0f).style("muted"));
            });
    ...
    private String secondaryStatus() {
        ...
        return currentTestFailed ? "Failed, moving to next test"
                : "Review visuals and controls";
    }
