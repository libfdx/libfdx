 ...
 * @author xpenatan
 */
public final class ShaderTest extends ApplicationAdapter {
     ...
     */
    public ShaderTest(long exitAfterFrames) {
    ...
    public void create(Fdx fdx) {
        ...
        logger = fdx.logger();
        fpsLogger = TestFpsLogger.create(logger, "ShaderTest");
        ...
        created = true;
        logger.info("ShaderTest created WGSL shader module and render pipeline for provider "
    ...
    public void dispose() {
        ...
        if (!created) {
            throw new FdxException("ShaderTest did not create graphics resources");
        }
        if (exitAfterFrames > 0L && renderedFrames < exitAfterFrames) {
            throw new FdxException("ShaderTest rendered " + renderedFrames + " of "
                    + exitAfterFrames + " required frames");
        }
        if (capturePath != null && capturePath.length() > 0 && !captured) {
            throw new FdxException("ShaderTest did not capture framebuffer to " + capturePath);
        }
        logger.info("ShaderTest rendered " + renderedFrames + " frames");
    }
    ...
    private void captureFrame(String path) {
        try {
            ...
            logger.info("ShaderTest captured framebuffer to " + path);
        } catch (Exception e) {
            throw new FdxException("Could not capture ShaderTest framebuffer", e);
        }
    }
