...
import io.github.libfdx.audio.web.WebAudioProvider;
...
import io.github.libfdx.tests.web.WebShaderCacheStoreTest;
 ...
 * @author xpenatan
 */
public final class WebTestLauncherSupport {
    ...
    public static void start(String runtimeName, String[] args, boolean webgpu,
        ...
        if (!cacheName.isEmpty()) {
            cacheStore = new WebShaderCacheStore(cacheName, 64 * 1024 * 1024);
            String fault = option(args, "shaderCacheFault", query("shaderCacheFault"), "");
            ShaderCacheStore storage = fault.isEmpty() ? cacheStore : faultStore(cacheStore, fault);
            shaderCache = new ShaderArtifactCache(storage);
            if (graphicsProvider instanceof WebWGPUProvider provider) {
                provider.configuration(new WGPUConfiguration().backend(WGPUBackend.WEBGPU).shaderCache(shaderCache));
            } else if (graphicsProvider instanceof WebGLProvider provider) {
                provider.shaderCache(shaderCache);
            } else throw new FdxException("Browser artifact-cache validation requires WebGPU or WebGL");
        }
        if (testName.startsWith("SceneShowcaseTest")) {
            config.deferAssets("streaming/", "atlas/", "deferred-image/",
                    UiFonts.DEFAULT_TTF_PATH);
        }
        if ("AudioPlaybackTest".equals(testName) || "MusicStreamingTest".equals(testName)
            ...
            config.audio(new WebAudioProvider());
        }
        if (capture.length() > 0) {
        ...
        if (hasQuery("autoChild")) {
            ManagedTestApplication managed = new ManagedTestApplication(selectedTest);
            backend.start(config, managed);
            awaitAutomaticCompletion(backend, managed);
        } else {
            backend.start(config, selectedTest);
        }
        if (cacheStore != null) observeShaderCache(backend, cacheStore, shaderCache);
    }
        ...
        return new ShaderCacheStore() {
            ...
            public FdxFuture<byte[]> readAsync(String key) {
                ...
                delegate.readAsync(key).onFailure(result::completeExceptionally).onSuccess(bytes -> {
                    if (bytes != null && bytes.length > 137) bytes[137] ^= 1;
    ...
    private static void observeShaderCache(WebApplicationBackend backend, WebShaderCacheStore store, ShaderArtifactCache cache) {
        Window.setTimeout(() -> {
            if (!backend.isDisposed()) { observeShaderCache(backend, store, cache); return; }
            store.flushAsync().onSuccess(ignored -> {
                System.out.println("[info] SHADER_CACHE layer=SOURCE " + cache.metrics(ShaderCacheLayer.SOURCE));
                System.out.println("[info] SHADER_CACHE layer=TRANSLATION " + cache.metrics(ShaderCacheLayer.TRANSLATION));
                store.dispose();
            }).onFailure(error -> { store.dispose(); throw new FdxException("Cache validation drain failed", error); });
        }, 100);
    }

    private static void awaitAutomaticCompletion(WebApplicationBackend backend, ManagedTestApplication managed) {
        Window.setTimeout(() -> {
            if (!backend.isDisposed()) {
                if (backend.lifecycle() == ApplicationLifecycle.RUNNING) publishAutomaticReady();
                awaitAutomaticCompletion(backend, managed);
                return;
            }
            try {
                ...
                publishAutomaticResult("FAIL", error.toString());
            }
        }, 100);
    }

    @JSBody(params = {"status", "detail"}, script = "window.libfdxAutoResult = {status: status, detail: detail};")
    private static native void publishAutomaticResult(String status, String detail);

    @JSBody(script = "window.libfdxAutoReady = true;")
    private static native void publishAutomaticReady();

    private static boolean isWebGPU(String graphics) {
        return "webgpu".equalsIgnoreCase(graphics) || "wgpu".equalsIgnoreCase(graphics);
    }

    public static boolean webGpuRequested(String[] args) {
    ...
    private static ApplicationListener test(String testName, long frames, String modelAsset, boolean webgpu) {
        ...
        if ("WebShaderCacheStoreTest".equalsIgnoreCase(testName)) return new WebShaderCacheStoreTest();
        if ("WebGPUShaderPreparationTest".equalsIgnoreCase(testName)) {
            if (!webgpu) throw new FdxException("WebGPUShaderPreparationTest requires WebGPU");
            return new WebGPUShaderPreparationTest();
        }
        if ("WebGPUDeviceLossTest".equalsIgnoreCase(testName)) {
            if (!webgpu) throw new FdxException("WebGPUDeviceLossTest requires WebGPU");
            return new WebGPUDeviceLossTest();
        }
        ...
        if ("ModelBatchTest".equalsIgnoreCase(testName)) {
    ...
    private static void applyTestProperties(String[] args) {
        setTestProperty(args, "autoDurationSeconds", "libfdx.test.autoDurationSeconds");
        setTestProperty(args, "shaderAsync", "libfdx.test.shaderAsync");
        setTestProperty(args, "shaderLoadingOnly", "libfdx.test.shaderLoadingOnly");
        setTestProperty(args, "shaderPreload", "libfdx.test.shaderPreload");
        setTestProperty(args, "shaderCount", "libfdx.test.shaderCount");
