...
val testAssetsDir = rootProject.layout.projectDirectory.dir("tests/assets")

fun registerWebBuild(taskName: String, descriptionText: String, target: String, mainClassName: String,
        title: String, outputPath: Provider<Directory>, optimization: String, assets: List<File>) {
    tasks.register(taskName) {
        ...
        doLast {
            runWebBuilder(builderClasspath, target, mainClassName, title, outputPath.get().asFile, optimization, assets)
        }
    }
}
...
registerWebBuild("test_webgl_js_build",
        "Builds the WebGL JavaScript test web application.",
        "js",
        "io.github.libfdx.tests.web.WebTestJsLauncher",
        "libfdx Tests - Web JS",
        jsWebappDir,
        "AGGRESSIVE",
        listOf(testAssetsDir.asFile))

registerWebBuild("test_webgl_wasm_build",
        "Builds the WebGL Wasm test web application.",
        "wasm",
        "io.github.libfdx.tests.web.WebTestWasmLauncher",
        "libfdx Tests - Web Wasm",
        wasmWebappDir,
        "BALANCED",
        listOf(testAssetsDir.asFile))
...
registerWebRun("test_webgl_js_run",
        "Builds and serves the WebGL JavaScript test web application.",
        "test_webgl_js_build", jsWebappDir, "/")

registerWebRun("test_webgl_wasm_run",
        "Builds and serves the WebGL Wasm test web application.",
        "test_webgl_wasm_build", wasmWebappDir, "/")

registerWebRun("test_webgpu_js_run",
        "Builds and serves the WebGPU JavaScript test web application.",
        "test_webgpu_js_build", jsWebappDir, "/webgpu.html")

registerWebRun("test_webgpu_wasm_run",
        "Builds and serves the WebGPU Wasm test web application.",
        "test_webgpu_wasm_build", wasmWebappDir, "/webgpu.html")
