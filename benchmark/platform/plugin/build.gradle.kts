...
dependencies {
    implementation(project(":benchmark:platform:desktop_c"))
}

val nativeTargetFileName = "libfdx-benchmark-desktop-c"
val nativeOpenConsole = providers.gradleProperty("libfdx.desktopC.openConsole")
...
fun nativeExecutable(modeSuffix: String): File {
...
val nativeBenchmarkGraphicsOptions = listOf(
    NativeBenchmarkGraphicsOption(
        ...
        "GL desktop_c",
        "GL",
        "GL uses the desktop_c GL provider and GLEW native resource module"
    ),
    NativeBenchmarkGraphicsOption(
        "vulkan",
        "vulkan",
...
fun registerNativeBenchmarkGraphicsMode(
    ...
    val nativeResultFile = layout.buildDirectory
        .file("benchmark-results/sprite-batch-stress/desktop-c-${graphicsOption.taskId}-$modeSuffix.properties")
        .get().asFile

    val runSpriteBatchStressNative = tasks.register<Exec>(
        "run_sprite_batch_stress_desktop_c_${graphicsOption.taskId}_$modeSuffix"
    ) {
        group = "benchmark"
        ...
        doFirst {
            ...
            val executable = nativeExecutable(modeSuffix)
    ...
    val generateSpriteBatchStressNativeReport = tasks.register(
        "generate_sprite_batch_stress_desktop_c_${graphicsOption.taskId}_report_$modeSuffix"
    ) {
        group = "benchmark"
        ...
        val reportFile = rootProject.layout.buildDirectory
            .file("reports/benchmark/desktop-c-${graphicsOption.taskId}-sprite-batch-stress-$modeSuffix.md")
            .get().asFile
        ...
        doLast {
            ...
            reportFile.writeText(buildString {
                appendLine("# Desktop C ${graphicsOption.reportName} SpriteBatch Stress Benchmark - $buildType")
                appendLine()
    ...
    tasks.register("benchmark_sprite_batch_stress_desktop_c_${graphicsOption.taskId}_$modeSuffix") {
        group = "benchmark"
    ...
    tasks.register("benchmark_desktop_c_${graphicsOption.taskId}_$modeSuffix") {
        group = "benchmark"
...
fun registerNativeBenchmarkMode(modeSuffix: String, buildType: String, nativeBuildTask: String) {
    ...
    tasks.register("benchmark_desktop_c_$modeSuffix") {
        group = "benchmark"
        description = "Runs the full desktop_c $buildType benchmark suite across GL and Vulkan and generates Markdown reports."
        dependsOn(reportTasks)
    }
}

registerNativeBenchmarkMode("debug", "Debug", "libfdx_desktop_c_build_debug")
registerNativeBenchmarkMode("release", "Release", "libfdx_desktop_c_build_release")
