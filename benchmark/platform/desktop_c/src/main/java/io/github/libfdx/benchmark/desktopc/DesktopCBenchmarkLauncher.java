package io.github.libfdx.benchmark.desktopc;
...
public final class DesktopCBenchmarkLauncher {
    private DesktopCBenchmarkLauncher() {
    }
    ...
    public static void main(String[] args) {
        ...
        System.out.println("[info] DesktopCBenchmarkLauncher starting " + benchmarkName
                + " with " + graphics.label
                + ", seconds=" + seconds
                + ", vSync=" + vSync
        ...
        new DesktopCApplicationBackend().start(config, benchmark(benchmarkName, seconds, result));
    }
    ...
    private static GraphicsSelection graphicsSelection(String value, boolean vSync) {
        ...
        if ("gl".equals(normalized) || "opengl".equals(normalized)) {
            return new GraphicsSelection("gl", "GL desktop_c", new DesktopCOpenGLProvider());
        }
        if (normalized.length() == 0 || "vk".equals(normalized) || "vulkan".equals(normalized)) {
            DesktopCVulkanProvider provider = new DesktopCVulkanProvider().vSync(vSync).framesInFlight(3);
            if (!vSync) {
                provider.configuration().preferMailboxPresentMode(false);
            }
            return new GraphicsSelection("vulkan", "Vulkan desktop_c", provider);
        }
        throw new FdxException("Unknown desktop_c benchmark graphics API: " + value
