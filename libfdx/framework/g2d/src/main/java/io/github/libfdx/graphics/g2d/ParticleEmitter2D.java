...
public final class ParticleEmitter2D {
    ...
    private float directionDegrees = 90.0f;
    private float spreadDegrees = 360.0f;
    ...
    public ParticleEmitter2D(int maxParticles) {
        if (maxParticles <= 0) {
            throw new FdxException("ParticleEmitter2D maxParticles must be greater than zero");
        }
    ...
    public ParticleEmitter2D position(float x, float y) {
        validateFinite(x, "ParticleEmitter2D x");
        validateFinite(y, "ParticleEmitter2D y");
        emitX = x;
        emitY = y;
        return this;
    }
    ...
    public ParticleEmitter2D emissionRate(float particlesPerSecond) {
        validateFinite(particlesPerSecond, "ParticleEmitter2D emission rate");
        if (particlesPerSecond < 0.0f) {
            throw new FdxException("ParticleEmitter2D emission rate cannot be negative");
        }
    ...
    public ParticleEmitter2D lifetime(float minSeconds, float maxSeconds) {
        validateRange(minSeconds, maxSeconds, true, "ParticleEmitter2D lifetime");
    ...
    public ParticleEmitter2D speed(float minUnitsPerSecond, float maxUnitsPerSecond) {
        validateRange(minUnitsPerSecond, maxUnitsPerSecond, false, "ParticleEmitter2D speed");
    ...
    /**
     * Sets the emission direction and spread and returns this emitter.
     *
     * @param degrees the center direction in degrees
     * @param spreadDegrees the total spread in degrees
     * @return this emitter
     */
    public ParticleEmitter2D direction(float degrees, float spreadDegrees) {
        validateFinite(degrees, "ParticleEmitter2D direction");
        validateFinite(spreadDegrees, "ParticleEmitter2D spread");
        if (spreadDegrees < 0.0f) {
            throw new FdxException("ParticleEmitter2D spread cannot be negative");
        }
    ...
    public ParticleEmitter2D gravity(float x, float y) {
        validateFinite(x, "ParticleEmitter2D gravity x");
        validateFinite(y, "ParticleEmitter2D gravity y");
        gravityX = x;
        gravityY = y;
        return this;
    }
    ...
    public ParticleEmitter2D size(float minStart, float maxStart, float minEnd, float maxEnd) {
        validateRange(minStart, maxStart, true, "ParticleEmitter2D start size");
        validateRange(minEnd, maxEnd, false, "ParticleEmitter2D end size");
    ...
    public ParticleEmitter2D color(float startRed, float startGreen, float startBlue, float startAlpha,
        ...
        validateColor(startRed, "ParticleEmitter2D start red");
        validateColor(startGreen, "ParticleEmitter2D start green");
        validateColor(startBlue, "ParticleEmitter2D start blue");
        validateColor(startAlpha, "ParticleEmitter2D start alpha");
        validateColor(endRed, "ParticleEmitter2D end red");
        validateColor(endGreen, "ParticleEmitter2D end green");
        validateColor(endBlue, "ParticleEmitter2D end blue");
        validateColor(endAlpha, "ParticleEmitter2D end alpha");
    ...
    public ParticleEmitter2D rotation(float minDegrees, float maxDegrees,
        ...
        validateOrderedRange(minDegrees, maxDegrees, "ParticleEmitter2D rotation");
        validateOrderedRange(minAngularVelocityDegrees, maxAngularVelocityDegrees,
                "ParticleEmitter2D angular velocity");
    ...
    public int update(float deltaSeconds) {
        validateFinite(deltaSeconds, "ParticleEmitter2D deltaSeconds");
        if (deltaSeconds < 0.0f) {
            throw new FdxException("ParticleEmitter2D deltaSeconds cannot be negative");
        }
    ...
    public int emit(int count) {
        if (count < 0) {
            throw new FdxException("ParticleEmitter2D emit count cannot be negative");
        }
        int emitted = 0;
    ...
    /**
     * Renders active particles through a caller-owned batch.
     *
     * @param region the particle texture region
     * @param batch the batch
     * @return the number of drawn particles
     */
    public int render(TextureRegion region, Batch2D batch) {
        if (region == null) {
            throw new FdxException("ParticleEmitter2D texture region cannot be null");
        }
        if (batch == null) {
            throw new FdxException("ParticleEmitter2D batch cannot be null");
        }
        int drawn = 0;
        for (int i = 0; i < activeCount; i++) {
            float size = size(i);
            if (size <= 0.0f || alpha(i) <= 0.0f) {
                continue;
            }
    ...
    private void spawn(int index) {
        ...
        velocityX[index] = (float)Math.cos(angle) * speed;
        velocityY[index] = (float)Math.sin(angle) * speed;
