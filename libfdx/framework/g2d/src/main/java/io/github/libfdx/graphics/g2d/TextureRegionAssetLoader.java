...
final class TextureRegionAssetLoader implements AssetLoader<TextureRegion> {
    ...
    public FdxFuture<TextureRegion> load(AssetLoadContext context, AssetDescriptor<TextureRegion> descriptor) {
        FdxFuture<Texture> texture = context.dependency(TextureLoadOptions.from(descriptor).descriptor(descriptor.path(), Texture.class));
        return context.completeOnUpdate(() -> new TextureRegion(texture.get()));
    }
}
