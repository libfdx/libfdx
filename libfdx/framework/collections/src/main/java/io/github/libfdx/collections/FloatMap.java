...
/**
 * Maps primitive float keys to object values without boxing keys.
 *
 * <p><b>Algorithm:</b> A power-of-two open-addressed hash table uses linear
 * probing and tombstones. Float keys are hashed and compared by their
 * canonical bit representation.</p>
 *
...
public final class FloatMap<V> {
     ...
     */
    public FloatMap(int capacity, float loadFactor) {
        if (capacity < 0) {
     ...
     */
    public float findKey(V value, float defaultKey) {
     ...
     * @return the key or default key
     */
    public float findKey(V value, boolean identity, float defaultKey) {
        for (int i = 0; i < states.length; i++) {
            if (states[i] == CollectionHash.USED && valueMatches(values[i], value, identity)) {
                return keys[i];
            }
     ...
     * @return this map
     */
    public FloatMap<V> ensureCapacity(int additionalCapacity) {
    ...
    private int locate(float key) {
        ...
        int index = CollectionHash.mix(keyBits) & mask;
        int firstRemoved = -1;
        for (int probes = 0; probes < keys.length; probes++) {
            ...
            if (state == CollectionHash.USED && Float.floatToIntBits(keys[index]) == keyBits) {
                return index;
            }
    ...
    /**
     * Represents a float-key map entry.
     *
     * @param <V> the value type
     * @author xpenatan
     */
