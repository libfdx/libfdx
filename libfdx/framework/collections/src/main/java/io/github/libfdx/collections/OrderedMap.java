 ...
 * @author xpenatan
 */
public final class OrderedMap<K, V> implements ObjectMapView<K, V> {
    ...
    /**
     * Creates an ordered map containing a copy of the supplied entries.
     *
     * @param values the entries
     */
    public OrderedMap(ObjectMapView<? extends K, ? extends V> values) {
     ...
     */
    public OrderedMap(int capacity, float loadFactor) {
        if (capacity < 0) {
    ...
    /**
     * Adds or replaces a value. Replacing a value keeps its original order.
     *
     * @param key the key
     ...
     * @return this map
     */
    public OrderedMap<K, V> putAll(ObjectMapView<? extends K, ? extends V> entries) {
        if (entries == null) {
    ...
    public boolean containsValue(V value, boolean identity) {
        for (int index = first; index >= 0; index = next[index]) {
            if (valueMatches(values[index], value, identity)) {
                return true;
            }
        }
        return false;
    ...
    /**
     * Removes a key in expected constant time.
     *
     * @param key the key
     * @return the previous value, or null
     */
     ...
     * @return this map
     */
    public OrderedMap<K, V> ensureCapacity(int additionalCapacity) {
    ...
    /**
     * Returns an insertion-ordered iterable view over entries.
     *
    ...
    /**
     * Returns an insertion-ordered iterable view over keys.
     *
     * @return the keys
     */
    @Override
    ...
    /**
     * Returns an insertion-ordered iterable view over values.
     *
     * @return the values
     */
    @Override
    ...
    public ObjectMapView<K, V> view() {
        if (view == null) {
            view = new ReadOnlyOrderedMapView<K, V>(this);
        }
        return view;
    }
    ...
    private void resize(int tableSize) {
        ...
        allocate(tableSize);
        size = 0;
        for (int index = oldFirst; index >= 0; index = oldNext[index]) {
            put(keyAt(oldKeys, index), valueAt(oldValues, index));
        }
        if (size != oldSize) {
    ...
    private void linkLast(int index) {
        previous[index] = last;
        next[index] = -1;
        if (last >= 0) {
            next[last] = index;
    ...
    @SuppressWarnings("unchecked")
    private K keyAt(Object[] source, int index) {
        return (K)source[index];
    }

    @SuppressWarnings("unchecked")
    private V valueAt(Object[] source, int index) {
    ...
    /**
     * Represents an ordered-map entry.
     *
     * @param <K> the key type
     * @param <V> the value type
     * @author xpenatan
    ...
    private static final class EntryIterator<K, V> implements ObjectIterator<Entry<K, V>> {
        ...
        EntryIterator(OrderedMap<K, V> map) {
            this.map = map;
        }
    ...
    private static final class KeyIterator<K, V> implements ObjectIterator<K> {
        ...
        KeyIterator(OrderedMap<K, V> map) {
            this.map = map;
        }
    ...
    private static final class ValueIterator<K, V> implements ObjectIterator<V> {
        ...
        ValueIterator(OrderedMap<K, V> map) {
            this.map = map;
        }
    ...
    private static final class ReadOnlyOrderedMapView<K, V> implements ObjectMapView<K, V> {
        private final OrderedMap<K, V> map;

        ReadOnlyOrderedMapView(OrderedMap<K, V> map) {
            this.map = map;
        }
