 ...
 */
public final class ParticlePresets3D {
    private ParticlePresets3D() {}

    /** Rising tapered flames; pair with ParticleSprite.FLAME. */
    public static ParticleEmitter3D fire(int capacity, float scale) {
        return base(capacity, scale).emissionRate(90).lifetime(0.65f, 1.15f)
                .speed(0.55f * scale, 1.05f * scale).direction(0, 1, 0, 24)
                .gravity(0, 0.3f * scale, 0).spawnArea(0.32f * scale, 0.04f * scale, 0.25f * scale)
    ...
    public static ParticleEmitter3D smoke(int capacity, float scale) {
        return base(capacity, scale).emissionRate(18).lifetime(2.2f, 3.4f)
                .speed(0.28f * scale, 0.45f * scale).direction(0, 1, 0, 32)
                .gravity(0.045f * scale, 0.08f * scale, 0)
                .spawnArea(0.25f * scale, 0.06f * scale, 0.25f * scale)
    ...
    public static ParticleEmitter3D sparks(int capacity, float scale) {
        return base(capacity, scale).emissionRate(28).lifetime(0.7f, 1.4f)
                .speed(0.8f * scale, 1.8f * scale).direction(0, 1, 0, 65)
                .gravity(0, -1.1f * scale, 0).size(0.10f * scale, 0.025f * scale)
                .color(1, 0.9f, 0.35f, 1, 1, 0.22f, 0.02f, 0).rotation(-25, 25, -60, 60);
    }

    /** Falling snow over a broad area; pair with ParticleSprite.SNOW. */
    public static ParticleEmitter3D snow(int capacity, float scale) {
        return base(capacity, scale).emissionRate(25).lifetime(3, 5)
                .speed(0.2f * scale, 0.4f * scale).direction(0, -1, 0, 18)
                .spawnArea(1.6f * scale, 0, 1.6f * scale)
    ...
    public static ParticleEmitter3D volumetricFire(int capacity, float scale) {
    ...
    private static ParticleEmitter3D base(int capacity, float scale) {
        if (!Float.isFinite(scale) || scale <= 0) throw new FdxException("Particle scale must be finite and positive");
        return new ParticleEmitter3D(capacity).curves(ParticleCurve.LINEAR, ParticleCurve.LINEAR, ParticleCurve.FADE);
    }
}
