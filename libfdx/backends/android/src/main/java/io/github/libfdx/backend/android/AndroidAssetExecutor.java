...
/**
 * Bounded Android workers for application-owned asset managers. Construct one
 ...
 */
public final class AndroidAssetExecutor implements AssetExecutor {
     ...
     */
    public AndroidAssetExecutor(int workers, int queuedTasks) {
        ...
        executor = new ThreadPoolExecutor(workers, workers, 0L, TimeUnit.MILLISECONDS,
                new ArrayBlockingQueue<Runnable>(queuedTasks), task -> {
                    Thread thread = new Thread(task, "libfdx-android-assets-" + nextWorker.incrementAndGet());
