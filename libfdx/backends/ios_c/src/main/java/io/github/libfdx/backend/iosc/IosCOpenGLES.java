...
/**
 * Exposes iOS OpenGLES symbols to TeaVM C.
 *
 * @author xpenatan
 */
@Include("OpenGLES/ES3/gl.h")
final class IosCOpenGLES {
    ...
    private static String toString(byte[] bytes, int length) {
        ...
        return actualLength == 0 ? "" : new String(bytes, 0, actualLength);
    }
    ...
    @Import(name = "libfdx_ios_c_glShaderSource")
    static native void libfdxIosCGlShaderSource(int shader, int length, Address sourceData);
    ...
    static native void glClearDepthf(float depth);

    @Import(name = "glClear")
    static native void glClear(int mask);

    @Import(name = "glReadPixels")
    static native void glReadPixels(int x, int y, int width, int height, int format, int type, ByteBuffer data);
