...
/**
 * Bounded application-owned disk cache. The supplied directory must be private to this cache;
 ...
 */
public final class DesktopShaderCacheStore implements ShaderCacheStore, Disposable {
    private final Path directory;
    private final long maxBytes;
    private final DesktopAssetExecutor executor = new DesktopAssetExecutor(1, 256);

    public DesktopShaderCacheStore(Path directory, long maxBytes) {
    ...
    private <T> FdxFuture<T> submit(FdxTask<T> task) {
        ...
        try {
            ...
            })) future.completeExceptionally(new FdxException("Shader cache I/O queue is full"));
