...
#ifdef _WIN32
        char buffer[8] = {};
        DWORD length = GetEnvironmentVariableA("LIBFDX_VULKAN_VERBOSE", buffer, sizeof(buffer));
        if (length == 0 || length >= sizeof(buffer)) {
            return false;
        }
        const char* value = buffer;
#else
        const char* value = std::getenv("LIBFDX_VULKAN_VERBOSE");
        if (value == nullptr) {
            return false;
        }
#endif
...
#define LOG_LINE(prefix, ...) do { \
    std::fprintf(stderr, prefix __VA_ARGS__); \
    std::fputc('\n', stderr); \
    std::fflush(stderr); \
} while (0)
#define LOGI(...) do { if (verboseLoggingEnabled()) { LOG_LINE("[libfdx-vulkan] ", __VA_ARGS__); } } while (0)
#define LOGW(...) LOG_LINE("[libfdx-vulkan] warn: ", __VA_ARGS__)
#define LOGE(...) LOG_LINE("[libfdx-vulkan] error: ", __VA_ARGS__)
...
bool loadInstanceFunction(VkInstance instance, T& target, const char* name) {
    ...
    if (target == nullptr) {
        LOGE("desktop C Vulkan loader did not provide %s", name);
        return false;
    }
    return true;
}
...
bool loadDeviceFunction(VkDevice device, T& target, const char* name) {
    ...
    if (target == nullptr) {
        LOGE("desktop C Vulkan device did not provide %s", name);
        return false;
    }
    return true;
}
...
void ensureVulkanLoader() {
    if (!loadVulkanLoader()) {
        throw std::runtime_error(vulkanLoaderError.empty()
                ? "Could not load desktop C Vulkan runtime"
                : vulkanLoaderError);
    }
...
bool waitForFenceOrTimeout(Context* context, VkFence fence, uint64_t timeoutNs, const char* reason) {
    ...
    if (waitResult == VK_TIMEOUT) {
        LOGW("desktop C Vulkan timed out waiting for %s", reason);
        return false;
    }
    check(waitResult, "Could not wait for desktop C Vulkan in-flight fence");
    return true;
}
...
void ensureNoActiveFramesOrThrow(Context* context, uint64_t timeoutNs, const char* reason) {
    if (!waitForActiveFramesOrTimeout(context, timeoutNs, reason)) {
        throw std::runtime_error(std::string("Timed out while waiting for desktop C Vulkan ")
                + reason);
    }
}
...
void recreateSignaledFrameFence(Context* context, FrameSync& frame) {
    ...
    check(vkCreateFence(context->device, &fenceInfo, nullptr, &replacement),
            "Could not recreate desktop C Vulkan in-flight fence");
    if (frame.inFlight != VK_NULL_HANDLE) {
...
VkApplicationInfo desktopCVulkanApplicationInfo() {
    VkApplicationInfo appInfo{};
    appInfo.sType = VK_STRUCTURE_TYPE_APPLICATION_INFO;
    appInfo.pApplicationName = "libfdx Desktop C";
...
std::vector<const char*> requiredInstanceExtensions() {
    ...
    if (extensions == nullptr || count == 0) {
        throw std::runtime_error("GLFW did not provide desktop C Vulkan instance extensions");
    }
...
std::vector<VkExtensionProperties> enumerateInstanceExtensions() {
    ...
    if (result != VK_SUCCESS) {
        throw std::runtime_error(std::string("Could not enumerate desktop C Vulkan instance extension count: ")
    ...
    if (count > 0) {
        ...
        if (result != VK_SUCCESS) {
            throw std::runtime_error(std::string("Could not enumerate desktop C Vulkan instance extensions: ")
                    + vkResultName(result) + " (" + std::to_string(result) + ")");
        }
    }
    return extensions;
...
void logInstanceCreateDiagnostics(const VkApplicationInfo& appInfo) {
    ...
    for (const std::vector<const char*>& attempt : attempts) {
        ...
        LOGI("desktop C Vulkan diagnostic vkCreateInstance extensions=[%s] result=%s (%d)",
                requestedExtensionNames(attempt).c_str(), vkResultName(result), result);
    }
}

std::string probeInstanceFailure() {
    VkApplicationInfo appInfo = desktopCVulkanApplicationInfo();
    ...
    for (const char* extension : requiredExtensions) {
        if (!containsExtension(availableExtensions, extension)) {
            return std::string("desktop C Vulkan is not usable because required instance extension ")
    ...
    return std::string("desktop C Vulkan is not usable on this device: vkCreateInstance failed with ")
...
VkSurfaceFormatKHR chooseSurfaceFormat(const std::vector<VkSurfaceFormatKHR>& formats) {
    if (formats.empty()) {
        throw std::runtime_error("desktop C Vulkan surface did not report any supported formats");
    }
...
VkPresentModeKHR choosePresentMode(VkPhysicalDevice device, VkSurfaceKHR surface, bool vSync,
    ...
    check(vkGetPhysicalDeviceSurfacePresentModesKHR(device, surface, &count, nullptr),
            "Could not get desktop C Vulkan present mode count");
    std::vector<VkPresentModeKHR> modes(count);
    if (count > 0) {
        check(vkGetPhysicalDeviceSurfacePresentModesKHR(device, surface, &count, modes.data()),
                "Could not get desktop C Vulkan present modes");
    }
...
VkRenderPass createRenderPass(Context* context, VkAttachmentLoadOp colorLoadOp,
    ...
    attachments[1].initialLayout = depthLoadOp == VK_ATTACHMENT_LOAD_OP_LOAD
    ...
    check(vkCreateRenderPass(context->device, &renderPassInfo, nullptr, &renderPass),
            "Could not create desktop C Vulkan render pass");
    return renderPass;
}
...
DepthAttachment createDepthAttachment(Context* context) {
    ...
    check(vkCreateImage(context->device, &imageInfo, nullptr, &depth.image),
            "Could not create desktop C Vulkan depth image");
    ...
    try {
        check(vkAllocateMemory(context->device, &allocationInfo, nullptr, &depth.memory),
                "Could not allocate desktop C Vulkan depth memory");
        check(vkBindImageMemory(context->device, depth.image, depth.memory, 0),
                "Could not bind desktop C Vulkan depth memory");
        ...
        check(vkCreateImageView(context->device, &viewInfo, nullptr, &depth.imageView),
                "Could not create desktop C Vulkan depth image view");
        return depth;
    } catch (...) {
...
void createSwapchain(Context* context) {
    ...
    LOGI("desktop C Vulkan swapchain selection: format=%d colorSpace=%d extent=%ux%u presentMode=%d minImages=%u maxImages=%u",
    ...
    check(vkCreateSwapchainKHR(context->device, &createInfo, nullptr, &context->swapchain),
            "Could not create desktop C Vulkan swapchain");

    uint32_t swapchainImageCount = 0;
    check(vkGetSwapchainImagesKHR(context->device, context->swapchain, &swapchainImageCount, nullptr),
            "Could not get desktop C Vulkan swapchain image count");
    LOGI("desktop C Vulkan swapchain created with %u images", swapchainImageCount);
    ...
    for (size_t i = 0; i < context->swapchainImages.size(); i++) {
        ...
        check(vkCreateImageView(context->device, &imageViewInfo, nullptr, &context->imageViews[i]),
                "Could not create desktop C Vulkan swapchain image view");
    }
    ...
    for (size_t i = 0; i < context->imageViews.size(); i++) {
        ...
        check(vkCreateFramebuffer(context->device, &framebufferInfo, nullptr, &context->framebuffers[i]),
                "Could not create desktop C Vulkan framebuffer");
    }
...
void recoverFailedFrame(Context* context, const char* operation) {
    ...
    try {
        ensureNoActiveFramesOrThrow(context, FRAME_FENCE_TIMEOUT_NS, "desktop C Vulkan frame recovery");
    } catch (const std::exception& error) {
        LOGW("Could not confirm desktop C Vulkan frame work completion after failed %s: %s",
    ...
    if (!context->frames.empty()) {
        ...
        if (resetResult != VK_SUCCESS) {
            LOGW("Could not reset desktop C Vulkan command buffer after failed %s: %d",
    ...
    try {
        ...
        LOGW("Could not recreate desktop C Vulkan swapchain after failed %s: %s",
                operation, error.what());
    }
}
...
void createInstance(Context* context) {
    ...
    std::vector<VkExtensionProperties> availableExtensions = enumerateInstanceExtensions();
    LOGI("desktop C Vulkan available instance extension count: %zu", availableExtensions.size());
    LOGI("desktop C Vulkan requested instance extensions: %s",
            requestedExtensionNames(extensions).c_str());
    for (const char* extension : extensions) {
        if (!containsExtension(availableExtensions, extension)) {
            throw std::runtime_error(std::string("Required desktop C Vulkan instance extension is missing: ")
    ...
    if (result != VK_SUCCESS) {
        ...
        throw std::runtime_error(std::string("Could not create desktop C Vulkan instance with extensions [")
                + requestedExtensionNames(extensions) + "]: " + vkResultName(result) + " ("
                + std::to_string(result) + ")");
    }
    loadVulkanInstanceFunctions(context->instance);
    LOGI("desktop C Vulkan instance created");
}

void createSurface(Context* context) {
    check(glfwCreateWindowSurface(context->instance, context->window, nullptr, &context->surface),
            "Could not create GLFW desktop C Vulkan surface");
    LOGI("desktop C Vulkan surface created");
}

void pickPhysicalDevice(Context* context) {
    uint32_t count = 0;
    check(vkEnumeratePhysicalDevices(context->instance, &count, nullptr),
            "Could not enumerate desktop C Vulkan physical device count");
    if (count == 0) {
        throw std::runtime_error("No desktop C Vulkan physical devices are available");
    }
    LOGI("desktop C Vulkan physical devices available: %u", count);
    std::vector<VkPhysicalDevice> devices(count);
    check(vkEnumeratePhysicalDevices(context->instance, &count, devices.data()),
            "Could not enumerate desktop C Vulkan physical devices");

    for (VkPhysicalDevice device : devices) {
        VkPhysicalDeviceProperties properties;
        vkGetPhysicalDeviceProperties(device, &properties);
        LOGI("desktop C Vulkan device candidate: %s api=%u.%u.%u driver=%u",
        ...
        if (!families.complete()) {
            LOGW("Skipping desktop C Vulkan device %s: missing graphics/present queue family",
                    properties.deviceName);
            continue;
        }
        LOGI("desktop C Vulkan device %s queue families: graphics=%u present=%u",
        ...
        LOGI("desktop C Vulkan device extension count for %s: %zu",
                properties.deviceName, extensions.size());
        if (!containsExtension(extensions, VK_KHR_SWAPCHAIN_EXTENSION_NAME)) {
            LOGW("Skipping desktop C Vulkan device %s: missing %s",
                    properties.deviceName, VK_KHR_SWAPCHAIN_EXTENSION_NAME);
            continue;
        }
        if (!supportsNegativeViewport(device)) {
            LOGW("Skipping desktop C Vulkan device %s: Vulkan 1.1 or %s required for negative viewport",
        ...
        LOGI("desktop C Vulkan selected physical device: %s", properties.deviceName);
        return;
    }
    throw std::runtime_error("No desktop C Vulkan device supports graphics, presentation, swapchain, and negative viewport");
}
...
void createDevice(Context* context) {
    ...
    if (result != VK_SUCCESS) {
        throw std::runtime_error(std::string("Could not create desktop C Vulkan logical device with extensions [")
                + requestedExtensionNames(extensions) + "]: " + vkResultName(result) + " ("
                + std::to_string(result) + ")");
    }
    ...
    LOGI("desktop C Vulkan logical device created");
}
...
VkDescriptorPool createFrameDescriptorPool(Context* context) {
    ...
    check(vkCreateDescriptorPool(context->device, &poolInfo, nullptr, &descriptorPool),
            "Could not create desktop C Vulkan frame descriptor pool");
...
void createCommandResources(Context* context, int framesInFlight) {
    ...
    check(vkCreateCommandPool(context->device, &poolInfo, nullptr, &context->commandPool),
            "Could not create desktop C Vulkan command pool");
    ...
    check(vkAllocateCommandBuffers(context->device, &allocateInfo, commandBuffers.data()),
            "Could not allocate desktop C Vulkan command buffers");
...
uint32_t findMemoryType(Context* context, uint32_t typeFilter, VkMemoryPropertyFlags properties) {
    ...
    throw std::runtime_error("Could not find suitable desktop C Vulkan memory type");
}
...
ReadbackBuffer createReadbackBuffer(Context* context, VkDeviceSize size) {
    ...
    check(vkCreateBuffer(context->device, &bufferInfo, nullptr, &readback.buffer),
            "Could not create desktop C Vulkan readback buffer");
    ...
    try {
        check(vkAllocateMemory(context->device, &allocationInfo, nullptr, &readback.memory),
                "Could not allocate desktop C Vulkan readback memory");
        check(vkBindBufferMemory(context->device, readback.buffer, readback.memory, 0),
                "Could not bind desktop C Vulkan readback memory");
        return readback;
    } catch (...) {
...
void transitionSwapchainImageLayout(VkCommandBuffer commandBuffer, VkImage image,
    ...
    if (oldLayout == VK_IMAGE_LAYOUT_PRESENT_SRC_KHR && newLayout == VK_IMAGE_LAYOUT_TRANSFER_SRC_OPTIMAL) {
        ...
        throw std::runtime_error("Unsupported desktop C Vulkan swapchain layout transition");
    }
...
void submitAndPresentFrame(Context* context, bool waitForCompletion) {
    ...
    check(vkEndCommandBuffer(frame.commandBuffer), "Could not end desktop C Vulkan command buffer");
    ...
    check(vkResetFences(context->device, 1, &frame.inFlight),
            "Could not reset desktop C Vulkan in-flight fence before submit");
    ...
    if (submitResult != VK_SUCCESS) {
        ...
        throw std::runtime_error("Could not submit desktop C Vulkan command buffer: "
    ...
    if (presentResult != VK_SUCCESS && presentResult != VK_ERROR_OUT_OF_DATE_KHR
        ...
        throw std::runtime_error("Could not present desktop C Vulkan frame: " + std::to_string(presentResult));
    }
...
void copyReadbackToJava(Context* context, ReadbackBuffer* readback, void* target, int targetSize) {
    ...
    if (targetSize < byteCount) {
        throw std::runtime_error("desktop C Vulkan readback target buffer is too small");
    }
    void* mapped = nullptr;
    check(vkMapMemory(context->device, readback->memory, 0, readback->size, 0, &mapped),
            "Could not map desktop C Vulkan readback memory");
    try {
...
Buffer* createBuffer(Context* context, int size, int usage) {
    if (size <= 0) {
        throw std::runtime_error("desktop C Vulkan buffer size must be greater than zero");
    }
    ...
    check(vkCreateBuffer(context->device, &bufferInfo, nullptr, &buffer->buffer),
            usage == 1 ? "Could not create desktop C Vulkan index buffer"
                    : "Could not create desktop C Vulkan vertex buffer");
    ...
    try {
        check(vkAllocateMemory(context->device, &allocationInfo, nullptr, &buffer->memory),
                "Could not allocate desktop C Vulkan vertex buffer memory");
        check(vkBindBufferMemory(context->device, buffer->buffer, buffer->memory, 0),
                "Could not bind desktop C Vulkan vertex buffer memory");
        check(vkMapMemory(context->device, buffer->memory, 0, buffer->size, 0, &buffer->mapped),
                "Could not map desktop C Vulkan vertex buffer memory");
        return buffer;
...
TransientBuffer createHostVisibleBuffer(Context* context, VkDeviceSize size, VkBufferUsageFlags usage,
    ...
    try {
        check(vkAllocateMemory(context->device, &allocationInfo, nullptr, &allocation.memory),
                "Could not allocate desktop C Vulkan host-visible buffer memory");
        check(vkBindBufferMemory(context->device, allocation.buffer, allocation.memory, 0),
                "Could not bind desktop C Vulkan host-visible buffer memory");
        return allocation;
    } catch (...) {
        if (allocation.memory != VK_NULL_HANDLE) {
            vkFreeMemory(context->device, allocation.memory, nullptr);
...
void copyToHostVisibleBuffer(Context* context, TransientBuffer* allocation, const void* source,
    ...
    check(vkMapMemory(context->device, allocation->memory, 0, size, 0, &mapped),
            "Could not map desktop C Vulkan host-visible buffer memory");
...
VkCommandBuffer beginSingleTimeCommands(Context* context) {
    ...
    check(vkAllocateCommandBuffers(context->device, &allocateInfo, &commandBuffer),
            "Could not allocate desktop C Vulkan one-time command buffer");
...
VkImageView createTextureImageView(Context* context, VkImage image, VkFormat format) {
    ...
    check(vkCreateImageView(context->device, &viewInfo, nullptr, &imageView),
            "Could not create desktop C Vulkan texture image view");
    return imageView;
}
...
VkSampler createTextureSampler(Context* context, int wrapS, int wrapT, int filter) {
    ...
    check(vkCreateSampler(context->device, &samplerInfo, nullptr, &sampler),
            "Could not create desktop C Vulkan texture sampler");
    return sampler;
}
...
Texture* createTextureResource(Context* context, int width, int height, VkFormat format, int wrapS, int wrapT,
    ...
    try {
        ...
        check(vkAllocateMemory(context->device, &allocationInfo, nullptr, &texture->memory),
                "Could not allocate desktop C Vulkan texture memory");
...
void writeTextureData(Texture* texture, const void* source, int size) {
    if (texture == nullptr || texture->context == nullptr) {
        throw std::runtime_error("desktop C Vulkan texture is not valid");
    }
    Context* context = texture->context;
    int byteCount = static_cast<int>(texture->width * texture->height * 4u);
    if (size != byteCount) {
        throw std::runtime_error("desktop C Vulkan texture upload expects "
    ...
    TransientBuffer staging = createHostVisibleBuffer(context, static_cast<VkDeviceSize>(byteCount),
            VK_BUFFER_USAGE_TRANSFER_SRC_BIT, "Could not create desktop C Vulkan texture staging buffer");
    try {
...
VkDescriptorSetLayout createTextureDescriptorSetLayout(Context* context, int sampledTextureCount) {
    ...
    check(vkCreateDescriptorSetLayout(context->device, &layoutInfo, nullptr, &layout),
            "Could not create desktop C Vulkan texture descriptor set layout");
    return layout;
}
...
VkDescriptorSetLayout createUniformDescriptorSetLayout(Context* context) {
    ...
    VkDescriptorSetLayout layout = VK_NULL_HANDLE;
    check(vkCreateDescriptorSetLayout(context->device, &layoutInfo, nullptr, &layout),
            "Could not create desktop C Vulkan uniform descriptor set layout");
    return layout;
}
...
void bindTextureDescriptors(Context* context, Pipeline* pipeline, Texture** textures, int count) {
    ...
    for (int i = 0; i < count; i++) {
        ...
        if (texture == nullptr) {
            throw std::runtime_error("desktop C Vulkan texture slot is not valid");
        }
...
extern "C" int32_t fdx_desktop_vulkan_probe_instance() {
    try {
        ...
        logNativeError("desktop C Vulkan instance probe failed", error);
        return 0;
    }
}
...
extern "C" int64_t fdx_desktop_vulkan_create(int64_t windowHandle, int32_t width, int32_t height,
    ...
    try {
        ...
        if (context->window == nullptr) {
            throw std::runtime_error("desktop C Vulkan requires a GLFW window handle");
        }
        ...
        logNativeError("Could not create desktop C Vulkan context", error);
        return 0;
    }
}
...
extern "C" void fdx_desktop_vulkan_resize(int64_t contextHandle, int32_t width, int32_t height) {
    ...
    try {
        ...
        logNativeError("Could not resize desktop C Vulkan context", error);
    }
}
...
extern "C" int32_t fdx_desktop_vulkan_begin_frame(int64_t contextHandle) {
    ...
    try {
        ...
        if (acquireResult == VK_TIMEOUT || acquireResult == VK_NOT_READY) {
            LOGW("desktop C Vulkan frame begin timed out waiting for swapchain image");
            return 0;
        }
        ...
        if (acquireResult != VK_SUCCESS && acquireResult != VK_SUBOPTIMAL_KHR) {
            throw std::runtime_error("Could not acquire desktop C Vulkan swapchain image: "
                    + std::to_string(acquireResult));
        }
        imageAcquired = true;

        check(vkResetCommandBuffer(frame.commandBuffer, 0), "Could not reset desktop C Vulkan command buffer");

        VkCommandBufferBeginInfo beginInfo{};
        beginInfo.sType = VK_STRUCTURE_TYPE_COMMAND_BUFFER_BEGIN_INFO;
        check(vkBeginCommandBuffer(frame.commandBuffer, &beginInfo),
                "Could not begin desktop C Vulkan command buffer");
        ...
        logNativeError("Could not begin desktop C Vulkan frame", error);
        return 0;
    }
}
...
extern "C" void fdx_desktop_vulkan_end_frame(int64_t contextHandle) {
    ...
    try {
        ...
        logNativeError("Could not end desktop C Vulkan frame", error);
    }
}
...
extern "C" void fdx_desktop_vulkan_read_pixels_rgba8(int64_t contextHandle, void* target, int32_t size) {
    ...
    try {
        if (!context->frameStarted) {
            throw std::runtime_error("Cannot read desktop C Vulkan pixels before beginFrame()");
        }
        if (!context->swapchainTransferSrcSupported) {
            throw std::runtime_error("desktop C Vulkan swapchain does not support transfer-source readback");
        }
        if (target == nullptr) {
            throw std::runtime_error("desktop C Vulkan readback requires a direct ByteBuffer");
        }
        ...
        logNativeError("Could not read desktop C Vulkan pixels", error);
    }
}
...
extern "C" void fdx_desktop_vulkan_clear(int64_t contextHandle, float red, float green, float blue, float alpha) {
    ...
    try {
        ...
        logNativeError("Could not clear desktop C Vulkan frame", error);
    }
}
...
extern "C" int64_t fdx_desktop_vulkan_create_buffer(int64_t contextHandle, int32_t size, int32_t usage) {
    ...
    try {
        ...
        logNativeError("Could not create desktop C Vulkan buffer", error);
        return 0;
    }
}
...
extern "C" void fdx_desktop_vulkan_write_buffer(int64_t bufferHandle, void* data, int32_t size) {
    ...
    try {
        if (buffer == nullptr || buffer->context == nullptr) {
            throw std::runtime_error("desktop C Vulkan buffer is not valid");
        }
        if (size < 0 || static_cast<VkDeviceSize>(size) > buffer->size) {
            throw std::runtime_error("desktop C Vulkan buffer data is larger than the destination buffer");
        }
        if (data == nullptr) {
            throw std::runtime_error("desktop C Vulkan buffer writes require a direct ByteBuffer");
        }
        ...
        logNativeError("Could not write desktop C Vulkan buffer", error);
    }
}
...
extern "C" void fdx_desktop_vulkan_write_texture(int64_t textureHandle, void* data, int32_t size) {
    ...
    try {
        if (data == nullptr) {
            throw std::runtime_error("desktop C Vulkan texture writes require a direct ByteBuffer");
        }
        writeTextureData(texture, data, static_cast<int>(size));
    } catch (const std::exception& error) {
        logNativeError("Could not write desktop C Vulkan texture", error);
    }
}
...
extern "C" int64_t fdx_desktop_vulkan_create_shader_module(int64_t contextHandle,
    ...
    try {
        ...
        delete module;
        logNativeError("Could not create desktop C Vulkan shader module", error);
        return 0;
    }
}
...
extern "C" int64_t fdx_desktop_vulkan_create_render_pipeline(int64_t contextHandle, int64_t shaderModuleHandle,
    ...
    try {
        ...
        if (vertexStrides.size() != vertexStepModes.size()) {
            throw std::runtime_error("desktop C Vulkan vertex layout arrays must have the same length");
        }
        if (attributeBindings.size() != attributeLocations.size()
            ...
            throw std::runtime_error("desktop C Vulkan vertex attribute arrays must have the same length");
        }
        ...
        for (size_t i = 0; i < vertexStrides.size(); i++) {
            if (vertexStrides[i] <= 0) {
                throw std::runtime_error("desktop C Vulkan vertex layout stride must be greater than zero");
            }
        ...
        for (size_t i = 0; i < attributeLocations.size(); i++) {
            ...
            if (binding < 0 || binding >= static_cast<int>(vertexStrides.size())) {
                throw std::runtime_error("desktop C Vulkan vertex attribute binding is out of range");
            }
        ...
        check(vkCreatePipelineLayout(context->device, &layoutInfo, nullptr, &pipeline->layout),
                "Could not create desktop C Vulkan pipeline layout");
        ...
        delete pipeline;
        logNativeError("Could not create desktop C Vulkan render pipeline", error);
        return 0;
    }
}
...
extern "C" void fdx_desktop_vulkan_begin_render_pass(int64_t contextHandle, int32_t clear,
    ...
    try {
        if (!context->frameStarted) {
            throw std::runtime_error("Cannot begin desktop C Vulkan render pass outside a frame");
        }
        if (context->renderPassStarted) {
            throw std::runtime_error("desktop C Vulkan render pass is already active");
        }
        VkClearValue clearValues[2]{};
        clearValues[0].color.float32[0] = red;
        clearValues[0].color.float32[1] = green;
        ...
        logNativeError("Could not begin desktop C Vulkan render pass", error);
    }
}
...
extern "C" void fdx_desktop_vulkan_set_pipeline(int64_t contextHandle, int64_t pipelineHandle) {
    ...
    try {
        if (!context->renderPassStarted) {
            throw std::runtime_error("Cannot bind desktop C Vulkan pipeline outside a render pass");
        }
        vkCmdBindPipeline(currentFrame(context).commandBuffer, VK_PIPELINE_BIND_POINT_GRAPHICS,
                pipeline->pipeline);
    } catch (const std::exception& error) {
        logNativeError("Could not bind desktop C Vulkan pipeline", error);
    }
}
...
extern "C" void fdx_desktop_vulkan_set_vertex_buffer(int64_t contextHandle, int32_t slot, int64_t bufferHandle) {
    ...
    try {
        if (!context->renderPassStarted) {
            throw std::runtime_error("Cannot bind desktop C Vulkan vertex buffer outside a render pass");
        }
        if (slot < 0) {
            throw std::runtime_error("desktop C Vulkan vertex buffer slot cannot be negative");
        }
        ...
        logNativeError("Could not bind desktop C Vulkan vertex buffer", error);
    }
}
...
extern "C" void fdx_desktop_vulkan_set_index_buffer(int64_t contextHandle, int64_t bufferHandle) {
    ...
    try {
        if (!context->renderPassStarted) {
            throw std::runtime_error("Cannot bind desktop C Vulkan index buffer outside a render pass");
        }
        vkCmdBindIndexBuffer(currentFrame(context).commandBuffer, buffer->buffer, 0, VK_INDEX_TYPE_UINT16);
    } catch (const std::exception& error) {
        logNativeError("Could not bind desktop C Vulkan index buffer", error);
    }
}
...
extern "C" void fdx_desktop_vulkan_set_viewport(int64_t contextHandle, int32_t x, int32_t y,
        int32_t width, int32_t height) {
    Context* context = ptr<Context>(contextHandle);
    try {
        if (!context->renderPassStarted) {
            throw std::runtime_error("Cannot set desktop C Vulkan viewport outside a render pass");
        }
        if (width <= 0 || height <= 0) {
            throw std::runtime_error("desktop C Vulkan viewport size must be greater than zero");
        }
        VkViewport viewport{};
        ...
        logNativeError("Could not set desktop C Vulkan viewport", error);
    }
}
...
extern "C" void fdx_desktop_vulkan_bind_textures(int64_t contextHandle, int64_t pipelineHandle,
    ...
    try {
        if (!context->renderPassStarted) {
            throw std::runtime_error("Cannot bind desktop C Vulkan textures outside a render pass");
        }
...
extern "C" int32_t fdx_desktop_vulkan_bind_uniforms(int64_t contextHandle, int64_t pipelineHandle,
    ...
    try {
        if (!context->renderPassStarted) {
            throw std::runtime_error("Cannot bind desktop C Vulkan uniforms outside a render pass");
        }
        if (data == nullptr) {
...
extern "C" void fdx_desktop_vulkan_draw(int64_t contextHandle, int32_t vertexCount,
    ...
    try {
        if (!context->renderPassStarted) {
            throw std::runtime_error("Cannot draw desktop C Vulkan geometry outside a render pass");
        }
        ...
        logNativeError("Could not draw desktop C Vulkan geometry", error);
    }
}
...
extern "C" void fdx_desktop_vulkan_draw_indexed(int64_t contextHandle, int32_t indexCount,
    ...
    try {
        if (!context->renderPassStarted) {
            throw std::runtime_error("Cannot draw indexed desktop C Vulkan geometry outside a render pass");
        }
        ...
        logNativeError("Could not draw indexed desktop C Vulkan geometry", error);
    }
}
...
extern "C" void fdx_desktop_vulkan_end_render_pass(int64_t contextHandle) {
    ...
    try {
        ...
        logNativeError("Could not end desktop C Vulkan render pass", error);
    }
}
