...
#else
#include "libfdx_vulkan_minimal.h"
#include <GLFW/glfw3.h>
#endif
...
#define LOGI(...) std::fprintf(stderr, "[libfdx-vulkan] " __VA_ARGS__), std::fprintf(stderr, "\n")
#define LOGW(...) std::fprintf(stderr, "[libfdx-vulkan] warn: " __VA_ARGS__), std::fprintf(stderr, "\n")
#define LOGE(...) std::fprintf(stderr, "[libfdx-vulkan] error: " __VA_ARGS__), std::fprintf(stderr, "\n")

#ifndef LIBFDX_VULKAN_FUNCTIONS
...
#define LIBFDX_DECLARE_VULKAN_POINTER(name) static decltype(&name) pfn_##name = nullptr;
LIBFDX_VULKAN_FUNCTIONS(LIBFDX_DECLARE_VULKAN_POINTER)
#undef LIBFDX_DECLARE_VULKAN_POINTER
...
#endif
bool vulkanLoaderAttempted = false;
std::string vulkanLoaderError;

template <typename T>
bool loadInstanceFunction(VkInstance instance, T& target, const char* name) {
    target = reinterpret_cast<T>(pfn_vkGetInstanceProcAddr(instance, name));
    if (target == nullptr) {
        LOGE("desktop native Vulkan loader did not provide %s", name);
        return false;
    }
    return true;
}
...
template <typename T>
bool loadDeviceFunction(VkDevice device, T& target, const char* name) {
    target = reinterpret_cast<T>(pfn_vkGetDeviceProcAddr(device, name));
    if (target == nullptr) {
        LOGE("desktop native Vulkan device did not provide %s", name);
        return false;
    }
    return true;
}
...
bool loadVulkanLoader() {
    ...
    if (vulkanLoaderAttempted) {
        return false;
    }
    vulkanLoaderAttempted = true;

#ifdef _WIN32
    vulkanLibrary = LoadLibraryA("vulkan-1.dll");
    if (vulkanLibrary == nullptr) {
        vulkanLoaderError = "Could not load vulkan-1.dll. Install a Vulkan runtime/driver.";
        return false;
    }
    pfn_vkGetInstanceProcAddr = reinterpret_cast<PFN_vkGetInstanceProcAddr>(
    ...
    if (vulkanLibrary == nullptr) {
        vulkanLoaderError = "Could not load libvulkan.so.1. Install a Vulkan runtime/driver.";
        return false;
    }
    pfn_vkGetInstanceProcAddr = reinterpret_cast<PFN_vkGetInstanceProcAddr>(
    ...
    if (pfn_vkGetInstanceProcAddr == nullptr) {
        vulkanLoaderError = "The Vulkan runtime did not export vkGetInstanceProcAddr.";
        return false;
    }

    glfwInitVulkanLoader(pfn_vkGetInstanceProcAddr);
    bool ok = true;
    ok &= loadInstanceFunction(VK_NULL_HANDLE, pfn_vkCreateInstance, "vkCreateInstance");
    ok &= loadInstanceFunction(VK_NULL_HANDLE, pfn_vkEnumerateInstanceExtensionProperties,
            "vkEnumerateInstanceExtensionProperties");
    if (!ok) {
        vulkanLoaderError = "The Vulkan runtime is missing required global entry points.";
    }
    return ok;
}

void ensureVulkanLoader() {
    if (!loadVulkanLoader()) {
        throw std::runtime_error(vulkanLoaderError.empty()
                ? "Could not load desktop native Vulkan runtime"
                : vulkanLoaderError);
    }
    if (!glfwVulkanSupported()) {
        throw std::runtime_error("GLFW reports that Vulkan is not available on this machine");
    }
}

void loadVulkanInstanceFunctions(VkInstance instance) {
    bool ok = true;
    ok &= loadInstanceFunction(instance, pfn_vkGetDeviceProcAddr, "vkGetDeviceProcAddr");
    ok &= loadInstanceFunction(instance, pfn_vkDestroyInstance, "vkDestroyInstance");
    ok &= loadInstanceFunction(instance, pfn_vkEnumeratePhysicalDevices, "vkEnumeratePhysicalDevices");
    ok &= loadInstanceFunction(instance, pfn_vkGetPhysicalDeviceProperties, "vkGetPhysicalDeviceProperties");
    ok &= loadInstanceFunction(instance, pfn_vkGetPhysicalDeviceQueueFamilyProperties,
            "vkGetPhysicalDeviceQueueFamilyProperties");
    ok &= loadInstanceFunction(instance, pfn_vkGetPhysicalDeviceMemoryProperties,
            "vkGetPhysicalDeviceMemoryProperties");
    ok &= loadInstanceFunction(instance, pfn_vkEnumerateDeviceExtensionProperties,
            "vkEnumerateDeviceExtensionProperties");
    ok &= loadInstanceFunction(instance, pfn_vkGetPhysicalDeviceSurfaceSupportKHR,
            "vkGetPhysicalDeviceSurfaceSupportKHR");
    ok &= loadInstanceFunction(instance, pfn_vkGetPhysicalDeviceSurfaceCapabilitiesKHR,
            "vkGetPhysicalDeviceSurfaceCapabilitiesKHR");
    ok &= loadInstanceFunction(instance, pfn_vkGetPhysicalDeviceSurfaceFormatsKHR,
            "vkGetPhysicalDeviceSurfaceFormatsKHR");
    ok &= loadInstanceFunction(instance, pfn_vkGetPhysicalDeviceSurfacePresentModesKHR,
            "vkGetPhysicalDeviceSurfacePresentModesKHR");
    ok &= loadInstanceFunction(instance, pfn_vkCreateDevice, "vkCreateDevice");
    ok &= loadInstanceFunction(instance, pfn_vkDestroySurfaceKHR, "vkDestroySurfaceKHR");
    if (!ok) {
        throw std::runtime_error("The Vulkan runtime is missing required instance entry points");
    }
}

void loadVulkanDeviceFunctions(VkDevice device) {
    bool ok = true;
    ok &= loadDeviceFunction(device, pfn_vkDestroyDevice, "vkDestroyDevice");
    ok &= loadDeviceFunction(device, pfn_vkGetDeviceQueue, "vkGetDeviceQueue");
    ok &= loadDeviceFunction(device, pfn_vkDeviceWaitIdle, "vkDeviceWaitIdle");
    ok &= loadDeviceFunction(device, pfn_vkCreateSwapchainKHR, "vkCreateSwapchainKHR");
    ok &= loadDeviceFunction(device, pfn_vkDestroySwapchainKHR, "vkDestroySwapchainKHR");
    ok &= loadDeviceFunction(device, pfn_vkGetSwapchainImagesKHR, "vkGetSwapchainImagesKHR");
    ok &= loadDeviceFunction(device, pfn_vkAcquireNextImageKHR, "vkAcquireNextImageKHR");
    ok &= loadDeviceFunction(device, pfn_vkQueuePresentKHR, "vkQueuePresentKHR");
    ok &= loadDeviceFunction(device, pfn_vkCreateImage, "vkCreateImage");
    ok &= loadDeviceFunction(device, pfn_vkDestroyImage, "vkDestroyImage");
    ok &= loadDeviceFunction(device, pfn_vkGetImageMemoryRequirements, "vkGetImageMemoryRequirements");
    ok &= loadDeviceFunction(device, pfn_vkBindImageMemory, "vkBindImageMemory");
    ok &= loadDeviceFunction(device, pfn_vkCreateImageView, "vkCreateImageView");
    ok &= loadDeviceFunction(device, pfn_vkDestroyImageView, "vkDestroyImageView");
    ok &= loadDeviceFunction(device, pfn_vkAllocateMemory, "vkAllocateMemory");
    ok &= loadDeviceFunction(device, pfn_vkFreeMemory, "vkFreeMemory");
    ok &= loadDeviceFunction(device, pfn_vkCreateRenderPass, "vkCreateRenderPass");
    ok &= loadDeviceFunction(device, pfn_vkDestroyRenderPass, "vkDestroyRenderPass");
    ok &= loadDeviceFunction(device, pfn_vkCreateFramebuffer, "vkCreateFramebuffer");
    ok &= loadDeviceFunction(device, pfn_vkDestroyFramebuffer, "vkDestroyFramebuffer");
    ok &= loadDeviceFunction(device, pfn_vkCreateCommandPool, "vkCreateCommandPool");
    ok &= loadDeviceFunction(device, pfn_vkDestroyCommandPool, "vkDestroyCommandPool");
    ok &= loadDeviceFunction(device, pfn_vkAllocateCommandBuffers, "vkAllocateCommandBuffers");
    ok &= loadDeviceFunction(device, pfn_vkFreeCommandBuffers, "vkFreeCommandBuffers");
    ok &= loadDeviceFunction(device, pfn_vkBeginCommandBuffer, "vkBeginCommandBuffer");
    ok &= loadDeviceFunction(device, pfn_vkEndCommandBuffer, "vkEndCommandBuffer");
    ok &= loadDeviceFunction(device, pfn_vkResetCommandBuffer, "vkResetCommandBuffer");
    ok &= loadDeviceFunction(device, pfn_vkCreateSemaphore, "vkCreateSemaphore");
    ok &= loadDeviceFunction(device, pfn_vkDestroySemaphore, "vkDestroySemaphore");
    ok &= loadDeviceFunction(device, pfn_vkCreateFence, "vkCreateFence");
    ok &= loadDeviceFunction(device, pfn_vkDestroyFence, "vkDestroyFence");
    ok &= loadDeviceFunction(device, pfn_vkWaitForFences, "vkWaitForFences");
    ok &= loadDeviceFunction(device, pfn_vkResetFences, "vkResetFences");
    ok &= loadDeviceFunction(device, pfn_vkCreateDescriptorPool, "vkCreateDescriptorPool");
    ok &= loadDeviceFunction(device, pfn_vkDestroyDescriptorPool, "vkDestroyDescriptorPool");
    ok &= loadDeviceFunction(device, pfn_vkResetDescriptorPool, "vkResetDescriptorPool");
    ok &= loadDeviceFunction(device, pfn_vkAllocateDescriptorSets, "vkAllocateDescriptorSets");
    ok &= loadDeviceFunction(device, pfn_vkUpdateDescriptorSets, "vkUpdateDescriptorSets");
    ok &= loadDeviceFunction(device, pfn_vkCreateDescriptorSetLayout, "vkCreateDescriptorSetLayout");
    ok &= loadDeviceFunction(device, pfn_vkDestroyDescriptorSetLayout, "vkDestroyDescriptorSetLayout");
    ok &= loadDeviceFunction(device, pfn_vkCreateBuffer, "vkCreateBuffer");
    ok &= loadDeviceFunction(device, pfn_vkDestroyBuffer, "vkDestroyBuffer");
    ok &= loadDeviceFunction(device, pfn_vkGetBufferMemoryRequirements, "vkGetBufferMemoryRequirements");
    ok &= loadDeviceFunction(device, pfn_vkBindBufferMemory, "vkBindBufferMemory");
    ok &= loadDeviceFunction(device, pfn_vkMapMemory, "vkMapMemory");
    ok &= loadDeviceFunction(device, pfn_vkUnmapMemory, "vkUnmapMemory");
    ok &= loadDeviceFunction(device, pfn_vkQueueSubmit, "vkQueueSubmit");
    ok &= loadDeviceFunction(device, pfn_vkQueueWaitIdle, "vkQueueWaitIdle");
    ok &= loadDeviceFunction(device, pfn_vkCreateShaderModule, "vkCreateShaderModule");
    ok &= loadDeviceFunction(device, pfn_vkDestroyShaderModule, "vkDestroyShaderModule");
    ok &= loadDeviceFunction(device, pfn_vkCreateSampler, "vkCreateSampler");
    ok &= loadDeviceFunction(device, pfn_vkDestroySampler, "vkDestroySampler");
    ...
    ok &= loadDeviceFunction(device, pfn_vkCmdSetViewport, "vkCmdSetViewport");
    ok &= loadDeviceFunction(device, pfn_vkCmdSetScissor, "vkCmdSetScissor");
    ok &= loadDeviceFunction(device, pfn_vkCmdBindPipeline, "vkCmdBindPipeline");
    ok &= loadDeviceFunction(device, pfn_vkCmdBindVertexBuffers, "vkCmdBindVertexBuffers");
    ok &= loadDeviceFunction(device, pfn_vkCmdBindIndexBuffer, "vkCmdBindIndexBuffer");
    ok &= loadDeviceFunction(device, pfn_vkCmdBindDescriptorSets, "vkCmdBindDescriptorSets");
    ok &= loadDeviceFunction(device, pfn_vkCmdDraw, "vkCmdDraw");
    ok &= loadDeviceFunction(device, pfn_vkCmdDrawIndexed, "vkCmdDrawIndexed");
    ok &= loadDeviceFunction(device, pfn_vkCmdPipelineBarrier, "vkCmdPipelineBarrier");
    ok &= loadDeviceFunction(device, pfn_vkCmdCopyImageToBuffer, "vkCmdCopyImageToBuffer");
    ok &= loadDeviceFunction(device, pfn_vkCmdCopyBufferToImage, "vkCmdCopyBufferToImage");
    if (!ok) {
        throw std::runtime_error("The Vulkan runtime is missing required device entry points");
    }
}
...
bool waitForFenceOrTimeout(Context* context, VkFence fence, uint64_t timeoutNs, const char* reason) {
    ...
    if (waitResult == VK_TIMEOUT) {
        LOGW("desktop native Vulkan timed out waiting for %s", reason);
        return false;
    }
    check(waitResult, "Could not wait for desktop native Vulkan in-flight fence");
    return true;
}
...
void ensureNoActiveFramesOrThrow(Context* context, uint64_t timeoutNs, const char* reason) {
    if (!waitForActiveFramesOrTimeout(context, timeoutNs, reason)) {
        throw std::runtime_error(std::string("Timed out while waiting for desktop native Vulkan ")
                + reason);
    }
}
...
void waitDeviceIdleBeforeDestroy(Context* context, const char* resourceName) {
    ...
    try {
        ...
        LOGW("Could not confirm desktop native Vulkan frame work completion before destroying %s",
                resourceName);
        LOGW("desktop native Vulkan destroy error: %s", error.what());
    }
}
...
void recreateSignaledFrameFence(Context* context, FrameSync& frame) {
    ...
    check(vkCreateFence(context->device, &fenceInfo, nullptr, &replacement),
            "Could not recreate desktop native Vulkan in-flight fence");
    if (frame.inFlight != VK_NULL_HANDLE) {
...
VkApplicationInfo desktopNativeVulkanApplicationInfo() {
    VkApplicationInfo appInfo{};
    appInfo.sType = VK_STRUCTURE_TYPE_APPLICATION_INFO;
    appInfo.pApplicationName = "libfdx Desktop Native";
...
std::vector<const char*> requiredInstanceExtensions() {
    ...
    if (extensions == nullptr || count == 0) {
        throw std::runtime_error("GLFW did not provide desktop native Vulkan instance extensions");
    }
...
std::vector<VkExtensionProperties> enumerateInstanceExtensions() {
    ...
    if (result != VK_SUCCESS) {
        throw std::runtime_error(std::string("Could not enumerate desktop native Vulkan instance extension count: ")
    ...
    if (count > 0) {
        ...
        if (result != VK_SUCCESS) {
            throw std::runtime_error(std::string("Could not enumerate desktop native Vulkan instance extensions: ")
                    + vkResultName(result) + " (" + std::to_string(result) + ")");
        }
    }
    return extensions;
...
void logInstanceCreateDiagnostics(const VkApplicationInfo& appInfo) {
    const std::vector<std::vector<const char*>> attempts = {{}, requiredInstanceExtensions()};
    for (const std::vector<const char*>& attempt : attempts) {
        VkResult result = tryCreateDiagnosticInstance(appInfo, attempt);
        LOGI("desktop native Vulkan diagnostic vkCreateInstance extensions=[%s] result=%s (%d)",
                requestedExtensionNames(attempt).c_str(), vkResultName(result), result);
    }
}

std::string probeInstanceFailure() {
    VkApplicationInfo appInfo = desktopNativeVulkanApplicationInfo();
    ...
    for (const char* extension : requiredExtensions) {
        if (!containsExtension(availableExtensions, extension)) {
            return std::string("desktop native Vulkan is not usable because required instance extension ")
    ...
    return std::string("desktop native Vulkan is not usable on this device: vkCreateInstance failed with ")
...
VkSurfaceFormatKHR chooseSurfaceFormat(const std::vector<VkSurfaceFormatKHR>& formats) {
    if (formats.empty()) {
        throw std::runtime_error("desktop native Vulkan surface did not report any supported formats");
    }
...
VkPresentModeKHR choosePresentMode(VkPhysicalDevice device, VkSurfaceKHR surface, bool vSync,
    ...
    check(vkGetPhysicalDeviceSurfacePresentModesKHR(device, surface, &count, nullptr),
            "Could not get desktop native Vulkan present mode count");
    std::vector<VkPresentModeKHR> modes(count);
    if (count > 0) {
        check(vkGetPhysicalDeviceSurfacePresentModesKHR(device, surface, &count, modes.data()),
                "Could not get desktop native Vulkan present modes");
    }
...
VkRenderPass createRenderPass(Context* context, VkAttachmentLoadOp colorLoadOp,
    ...
    check(vkCreateRenderPass(context->device, &renderPassInfo, nullptr, &renderPass),
            "Could not create desktop native Vulkan render pass");
    return renderPass;
}
...
DepthAttachment createDepthAttachment(Context* context) {
    ...
    check(vkCreateImage(context->device, &imageInfo, nullptr, &depth.image),
            "Could not create desktop native Vulkan depth image");
    ...
    try {
        check(vkAllocateMemory(context->device, &allocationInfo, nullptr, &depth.memory),
                "Could not allocate desktop native Vulkan depth memory");
        check(vkBindImageMemory(context->device, depth.image, depth.memory, 0),
                "Could not bind desktop native Vulkan depth memory");
        ...
        check(vkCreateImageView(context->device, &viewInfo, nullptr, &depth.imageView),
                "Could not create desktop native Vulkan depth image view");
        return depth;
    } catch (...) {
...
void createSwapchain(Context* context) {
    ensureNoActiveFramesOrThrow(context, FRAME_FENCE_TIMEOUT_NS,
            "desktop native Vulkan swapchain recreation");
    ...
    LOGI("desktop native Vulkan swapchain selection: format=%d colorSpace=%d extent=%ux%u presentMode=%d minImages=%u maxImages=%u",
    ...
    check(vkCreateSwapchainKHR(context->device, &createInfo, nullptr, &context->swapchain),
            "Could not create desktop native Vulkan swapchain");

    uint32_t swapchainImageCount = 0;
    check(vkGetSwapchainImagesKHR(context->device, context->swapchain, &swapchainImageCount, nullptr),
            "Could not get desktop native Vulkan swapchain image count");
    LOGI("desktop native Vulkan swapchain created with %u images", swapchainImageCount);
    ...
    check(vkGetSwapchainImagesKHR(context->device, context->swapchain, &swapchainImageCount,
            context->swapchainImages.data()), "Could not get desktop native Vulkan swapchain images");
    ...
    for (size_t i = 0; i < context->swapchainImages.size(); i++) {
        ...
        check(vkCreateImageView(context->device, &imageViewInfo, nullptr, &context->imageViews[i]),
                "Could not create desktop native Vulkan swapchain image view");
    }
    ...
    for (size_t i = 0; i < context->imageViews.size(); i++) {
        ...
        check(vkCreateFramebuffer(context->device, &framebufferInfo, nullptr, &context->framebuffers[i]),
                "Could not create desktop native Vulkan framebuffer");
    }
...
void recoverFailedFrame(Context* context, const char* operation) {
    ...
    try {
        ensureNoActiveFramesOrThrow(context, FRAME_FENCE_TIMEOUT_NS, "desktop native Vulkan frame recovery");
    } catch (const std::exception& error) {
        LOGW("Could not confirm desktop native Vulkan frame work completion after failed %s: %s",
    ...
    if (!context->frames.empty()) {
        ...
        if (resetResult != VK_SUCCESS) {
            LOGW("Could not reset desktop native Vulkan command buffer after failed %s: %d",
    ...
    try {
        ...
        LOGW("Could not recreate desktop native Vulkan swapchain after failed %s: %s",
                operation, error.what());
    }
}
...
void createInstance(Context* context) {
    ...
    std::vector<VkExtensionProperties> availableExtensions = enumerateInstanceExtensions();
    LOGI("desktop native Vulkan available instance extension count: %zu", availableExtensions.size());
    LOGI("desktop native Vulkan requested instance extensions: %s",
            requestedExtensionNames(extensions).c_str());
    for (const char* extension : extensions) {
        if (!containsExtension(availableExtensions, extension)) {
            throw std::runtime_error(std::string("Required desktop native Vulkan instance extension is missing: ")
    ...
    if (result != VK_SUCCESS) {
        ...
        throw std::runtime_error(std::string("Could not create desktop native Vulkan instance with extensions [")
                + requestedExtensionNames(extensions) + "]: " + vkResultName(result) + " ("
                + std::to_string(result) + ")");
    }
    loadVulkanInstanceFunctions(context->instance);
    LOGI("desktop native Vulkan instance created");
}

void createSurface(Context* context) {
    check(glfwCreateWindowSurface(context->instance, context->window, nullptr, &context->surface),
            "Could not create GLFW desktop native Vulkan surface");
    LOGI("desktop native Vulkan surface created");
}

void pickPhysicalDevice(Context* context) {
    uint32_t count = 0;
    check(vkEnumeratePhysicalDevices(context->instance, &count, nullptr),
            "Could not enumerate desktop native Vulkan physical device count");
    if (count == 0) {
        throw std::runtime_error("No desktop native Vulkan physical devices are available");
    }
    LOGI("desktop native Vulkan physical devices available: %u", count);
    std::vector<VkPhysicalDevice> devices(count);
    check(vkEnumeratePhysicalDevices(context->instance, &count, devices.data()),
            "Could not enumerate desktop native Vulkan physical devices");

    for (VkPhysicalDevice device : devices) {
        VkPhysicalDeviceProperties properties;
        vkGetPhysicalDeviceProperties(device, &properties);
        LOGI("desktop native Vulkan device candidate: %s api=%u.%u.%u driver=%u",
        ...
        if (!families.complete()) {
            LOGW("Skipping desktop native Vulkan device %s: missing graphics/present queue family",
                    properties.deviceName);
            continue;
        }
        LOGI("desktop native Vulkan device %s queue families: graphics=%u present=%u",
        ...
        LOGI("desktop native Vulkan device extension count for %s: %zu",
                properties.deviceName, extensions.size());
        if (!containsExtension(extensions, VK_KHR_SWAPCHAIN_EXTENSION_NAME)) {
            LOGW("Skipping desktop native Vulkan device %s: missing %s",
                    properties.deviceName, VK_KHR_SWAPCHAIN_EXTENSION_NAME);
            continue;
        }
        if (!supportsNegativeViewport(device)) {
            LOGW("Skipping desktop native Vulkan device %s: Vulkan 1.1 or %s required for negative viewport",
        ...
        LOGI("desktop native Vulkan selected physical device: %s", properties.deviceName);
        return;
    }
    throw std::runtime_error("No desktop native Vulkan device supports graphics, presentation, swapchain, and negative viewport");
}
...
void createDevice(Context* context) {
    ...
    LOGI("desktop native Vulkan requested device extensions: %s", requestedExtensionNames(extensions).c_str());
    ...
    if (result != VK_SUCCESS) {
        throw std::runtime_error(std::string("Could not create desktop native Vulkan logical device with extensions [")
                + requestedExtensionNames(extensions) + "]: " + vkResultName(result) + " ("
                + std::to_string(result) + ")");
    }
    loadVulkanDeviceFunctions(context->device);
    ...
    LOGI("desktop native Vulkan logical device created");
}
...
VkDescriptorPool createFrameDescriptorPool(Context* context) {
    ...
    check(vkCreateDescriptorPool(context->device, &poolInfo, nullptr, &descriptorPool),
            "Could not create desktop native Vulkan frame descriptor pool");
...
void createCommandResources(Context* context, int framesInFlight) {
    ...
    check(vkCreateCommandPool(context->device, &poolInfo, nullptr, &context->commandPool),
            "Could not create desktop native Vulkan command pool");
    ...
    check(vkAllocateCommandBuffers(context->device, &allocateInfo, commandBuffers.data()),
            "Could not allocate desktop native Vulkan command buffers");
    ...
    for (size_t i = 0; i < context->frames.size(); i++) {
        ...
        check(vkCreateSemaphore(context->device, &semaphoreInfo, nullptr, &context->frames[i].imageAvailable),
                "Could not create desktop native Vulkan image-available semaphore");
        check(vkCreateSemaphore(context->device, &semaphoreInfo, nullptr, &context->frames[i].renderFinished),
                "Could not create desktop native Vulkan render-finished semaphore");
        check(vkCreateFence(context->device, &fenceInfo, nullptr, &context->frames[i].inFlight),
                "Could not create desktop native Vulkan in-flight fence");
...
void destroyContext(Context* context) {
    ...
    if (context->device != VK_NULL_HANDLE) {
        ensureNoActiveFramesOrThrow(context, FRAME_FENCE_TIMEOUT_NS, "desktop native Vulkan context destroy");
        destroySwapchainResources(context);
        for (FrameSync& frame : context->frames) {
...
uint32_t findMemoryType(Context* context, uint32_t typeFilter, VkMemoryPropertyFlags properties) {
    ...
    throw std::runtime_error("Could not find suitable desktop native Vulkan memory type");
}
...
ReadbackBuffer createReadbackBuffer(Context* context, VkDeviceSize size) {
    ...
    check(vkCreateBuffer(context->device, &bufferInfo, nullptr, &readback.buffer),
            "Could not create desktop native Vulkan readback buffer");
    ...
    try {
        check(vkAllocateMemory(context->device, &allocationInfo, nullptr, &readback.memory),
                "Could not allocate desktop native Vulkan readback memory");
        check(vkBindBufferMemory(context->device, readback.buffer, readback.memory, 0),
                "Could not bind desktop native Vulkan readback memory");
        return readback;
    } catch (...) {
...
void transitionSwapchainImageLayout(VkCommandBuffer commandBuffer, VkImage image,
    ...
    if (oldLayout == VK_IMAGE_LAYOUT_PRESENT_SRC_KHR && newLayout == VK_IMAGE_LAYOUT_TRANSFER_SRC_OPTIMAL) {
        ...
        throw std::runtime_error("Unsupported desktop native Vulkan swapchain layout transition");
    }
...
void submitAndPresentFrame(Context* context, bool waitForCompletion) {
    ...
    check(vkEndCommandBuffer(frame.commandBuffer), "Could not end desktop native Vulkan command buffer");
    ...
    check(vkResetFences(context->device, 1, &frame.inFlight),
            "Could not reset desktop native Vulkan in-flight fence before submit");
    ...
    if (submitResult != VK_SUCCESS) {
        ...
        throw std::runtime_error("Could not submit desktop native Vulkan command buffer: "
                + std::to_string(submitResult));
    }
    if (waitForCompletion) {
        if (!waitForFenceOrTimeout(context, frame.inFlight, READBACK_FENCE_TIMEOUT_NS,
                "desktop native Vulkan readback fence")) {
            throw std::runtime_error("Could not wait for desktop native Vulkan readback fence");
        }
    }
    ...
    if (presentResult != VK_SUCCESS && presentResult != VK_ERROR_OUT_OF_DATE_KHR
        ...
        throw std::runtime_error("Could not present desktop native Vulkan frame: " + std::to_string(presentResult));
    }
...
void copyReadbackToJava(Context* context, ReadbackBuffer* readback, void* target, int targetSize) {
    ...
    if (targetSize < byteCount) {
        throw std::runtime_error("desktop native Vulkan readback target buffer is too small");
    }
    void* mapped = nullptr;
    check(vkMapMemory(context->device, readback->memory, 0, readback->size, 0, &mapped),
            "Could not map desktop native Vulkan readback memory");
    try {
...
std::vector<int> intsFromPointer(const int32_t* array, int length, const char* name, bool allowEmpty) {
    if (length <= 0) {
        if (allowEmpty) {
            return {};
        }
    ...
    for (int i = 0; i < length; i++) {
        values[static_cast<size_t>(i)] = static_cast<int>(array[i]);
    }
    return values;
}

std::vector<uint32_t> wordsFromPointer(const int32_t* array, int length, const char* name) {
    if (array == nullptr) {
        throw std::runtime_error(std::string(name) + " cannot be null");
    }
    if (length <= 0) {
    ...
    for (int i = 0; i < length; i++) {
        words[static_cast<size_t>(i)] = static_cast<uint32_t>(array[i]);
    }
    return words;
}
...
VkShaderModule createShaderModule(Context* context, const std::vector<uint32_t>& words) {
    ...
    check(vkCreateShaderModule(context->device, &createInfo, nullptr, &shaderModule),
            "Could not create desktop native Vulkan shader module");
    return shaderModule;
}

Buffer* createBuffer(Context* context, int size, int usage) {
    if (size <= 0) {
        throw std::runtime_error("desktop native Vulkan buffer size must be greater than zero");
    }
    ...
    check(vkCreateBuffer(context->device, &bufferInfo, nullptr, &buffer->buffer),
            usage == 1 ? "Could not create desktop native Vulkan index buffer"
                    : "Could not create desktop native Vulkan vertex buffer");
    ...
    try {
        check(vkAllocateMemory(context->device, &allocationInfo, nullptr, &buffer->memory),
                "Could not allocate desktop native Vulkan vertex buffer memory");
        check(vkBindBufferMemory(context->device, buffer->buffer, buffer->memory, 0),
                "Could not bind desktop native Vulkan vertex buffer memory");
        check(vkMapMemory(context->device, buffer->memory, 0, buffer->size, 0, &buffer->mapped),
                "Could not map desktop native Vulkan vertex buffer memory");
        return buffer;
...
TransientBuffer createHostVisibleBuffer(Context* context, VkDeviceSize size, VkBufferUsageFlags usage,
    ...
    try {
        check(vkAllocateMemory(context->device, &allocationInfo, nullptr, &allocation.memory),
                "Could not allocate desktop native Vulkan host-visible buffer memory");
        check(vkBindBufferMemory(context->device, allocation.buffer, allocation.memory, 0),
                "Could not bind desktop native Vulkan host-visible buffer memory");
        return allocation;
    } catch (...) {
        if (allocation.memory != VK_NULL_HANDLE) {
            vkFreeMemory(context->device, allocation.memory, nullptr);
...
void copyToHostVisibleBuffer(Context* context, TransientBuffer* allocation, const void* source,
    ...
    check(vkMapMemory(context->device, allocation->memory, 0, size, 0, &mapped),
            "Could not map desktop native Vulkan host-visible buffer memory");
...
VkCommandBuffer beginSingleTimeCommands(Context* context) {
    ...
    check(vkAllocateCommandBuffers(context->device, &allocateInfo, &commandBuffer),
            "Could not allocate desktop native Vulkan one-time command buffer");

    VkCommandBufferBeginInfo beginInfo{};
    beginInfo.sType = VK_STRUCTURE_TYPE_COMMAND_BUFFER_BEGIN_INFO;
    beginInfo.flags = VK_COMMAND_BUFFER_USAGE_ONE_TIME_SUBMIT_BIT;
    check(vkBeginCommandBuffer(commandBuffer, &beginInfo),
            "Could not begin desktop native Vulkan one-time command buffer");
    return commandBuffer;
}

void endSingleTimeCommands(Context* context, VkCommandBuffer commandBuffer) {
    check(vkEndCommandBuffer(commandBuffer), "Could not end desktop native Vulkan one-time command buffer");
    ...
    check(vkCreateFence(context->device, &fenceInfo, nullptr, &fence),
            "Could not create desktop native Vulkan one-time command fence");

    try {
        check(vkQueueSubmit(context->graphicsQueue, 1, &submitInfo, fence),
                "Could not submit desktop native Vulkan one-time command buffer");
        if (!waitForFenceOrTimeout(context, fence, SINGLE_COMMAND_TIMEOUT_NS,
                "desktop native Vulkan one-time command fence")) {
            throw std::runtime_error("Could not wait for desktop native Vulkan one-time command fence");
        }
    } catch (...) {
...
VkImageView createTextureImageView(Context* context, VkImage image, VkFormat format) {
    ...
    check(vkCreateImageView(context->device, &viewInfo, nullptr, &imageView),
            "Could not create desktop native Vulkan texture image view");
    return imageView;
}
...
VkSampler createTextureSampler(Context* context, int wrapS, int wrapT) {
    ...
    check(vkCreateSampler(context->device, &samplerInfo, nullptr, &sampler),
            "Could not create desktop native Vulkan texture sampler");
    return sampler;
}

Texture* createTextureResource(Context* context, int width, int height, VkFormat format, int wrapS, int wrapT) {
    if (width <= 0 || height <= 0) {
        throw std::runtime_error("desktop native Vulkan texture size must be greater than zero");
    }
    if (format != VK_FORMAT_R8G8B8A8_UNORM) {
        throw std::runtime_error("desktop native Vulkan currently supports RGBA8_UNORM sampled textures only");
    }
    ...
    try {
        ...
        check(vkCreateImage(context->device, &imageInfo, nullptr, &texture->image),
                "Could not create desktop native Vulkan texture image");
        ...
        check(vkAllocateMemory(context->device, &allocationInfo, nullptr, &texture->memory),
                "Could not allocate desktop native Vulkan texture memory");
        check(vkBindImageMemory(context->device, texture->image, texture->memory, 0),
                "Could not bind desktop native Vulkan texture memory");
...
void transitionTextureLayout(VkCommandBuffer commandBuffer, Texture* texture,
    ...
    if (oldLayout == VK_IMAGE_LAYOUT_UNDEFINED
        ...
        throw std::runtime_error("Unsupported desktop native Vulkan texture layout transition");
    }
...
void writeTextureData(Texture* texture, const void* source, int size) {
    if (texture == nullptr || texture->context == nullptr) {
        throw std::runtime_error("desktop native Vulkan texture is not valid");
    }
    Context* context = texture->context;
    int byteCount = static_cast<int>(texture->width * texture->height * 4u);
    if (size != byteCount) {
        throw std::runtime_error("desktop native Vulkan texture upload expects "
    ...
    TransientBuffer staging = createHostVisibleBuffer(context, static_cast<VkDeviceSize>(byteCount),
            VK_BUFFER_USAGE_TRANSFER_SRC_BIT, "Could not create desktop native Vulkan texture staging buffer");
    try {
...
VkDescriptorSet allocateDescriptorSet(Context* context, VkDescriptorSetLayout layout) {
    ...
    check(vkAllocateDescriptorSets(context->device, &allocateInfo, &descriptorSet),
            "Could not allocate desktop native Vulkan descriptor set");
...
VkDescriptorSetLayout createTextureDescriptorSetLayout(Context* context, int sampledTextureCount) {
    ...
    check(vkCreateDescriptorSetLayout(context->device, &layoutInfo, nullptr, &layout),
            "Could not create desktop native Vulkan texture descriptor set layout");
    return layout;
}
...
VkDescriptorSetLayout createUniformDescriptorSetLayout(Context* context) {
    ...
    VkDescriptorSetLayout layout = VK_NULL_HANDLE;
    check(vkCreateDescriptorSetLayout(context->device, &layoutInfo, nullptr, &layout),
            "Could not create desktop native Vulkan uniform descriptor set layout");
    return layout;
}
...
void bindTextureDescriptors(Context* context, Pipeline* pipeline, Texture** textures, int count) {
    ...
    if (count != pipeline->sampledTextureCount) {
        throw std::runtime_error("desktop native Vulkan texture count does not match the active pipeline");
    }
    ...
    for (int i = 0; i < count; i++) {
        ...
        if (texture == nullptr) {
            throw std::runtime_error("desktop native Vulkan texture slot is not valid");
        }
...
void bindUniformDescriptor(Context* context, Pipeline* pipeline, const void* source, int size) {
    ...
    if (size != PBR_UNIFORM_BYTE_COUNT) {
        throw std::runtime_error("desktop native Vulkan PBR uniform upload has an unexpected size");
    }
    TransientBuffer uniformBuffer = createHostVisibleBuffer(context, PBR_UNIFORM_BYTE_COUNT,
            VK_BUFFER_USAGE_UNIFORM_BUFFER_BIT, "Could not create desktop native Vulkan uniform buffer");
    try {
...
extern "C" int32_t fdx_desktop_vulkan_probe_instance() {
    try {
        ...
        logNativeError("desktop native Vulkan instance probe failed", error);
        return 0;
    }
}

extern "C" int64_t fdx_desktop_vulkan_create(int64_t windowHandle, int32_t width, int32_t height,
        int32_t vSync, int32_t preferMailboxPresentMode, int32_t framesInFlight) {
    Context* context = new Context();
    try {
        context->window = reinterpret_cast<GLFWwindow*>(static_cast<intptr_t>(windowHandle));
        if (context->window == nullptr) {
            throw std::runtime_error("desktop native Vulkan requires a GLFW window handle");
        }
        context->requestedWidth = std::max(1, static_cast<int>(width));
        context->requestedHeight = std::max(1, static_cast<int>(height));
        context->vSync = vSync != 0;
        context->preferMailboxPresentMode = preferMailboxPresentMode != 0;
        ...
        logNativeError("Could not create desktop native Vulkan context", error);
        return 0;
    }
}

extern "C" void fdx_desktop_vulkan_resize(int64_t contextHandle, int32_t width, int32_t height) {
    ...
    try {
        ...
        logNativeError("Could not resize desktop native Vulkan context", error);
    }
}

extern "C" int32_t fdx_desktop_vulkan_begin_frame(int64_t contextHandle) {
    Context* context = ptr<Context>(contextHandle);
    ...
    try {
        ...
        FrameSync& frame = currentFrame(context);
        if (!waitForFenceOrTimeout(context, frame.inFlight, FRAME_FENCE_TIMEOUT_NS, "desktop native Vulkan in-flight fence")) {
            return 0;
        }
        destroyTransientBuffers(context, frame);
        if (frame.descriptorPool != VK_NULL_HANDLE) {
            check(vkResetDescriptorPool(context->device, frame.descriptorPool, 0),
                    "Could not reset desktop native Vulkan frame descriptor pool");
        }
        ...
        if (acquireResult == VK_TIMEOUT || acquireResult == VK_NOT_READY) {
            LOGW("desktop native Vulkan frame begin timed out waiting for swapchain image");
            return 0;
        }
        ...
        if (acquireResult != VK_SUCCESS && acquireResult != VK_SUBOPTIMAL_KHR) {
            throw std::runtime_error("Could not acquire desktop native Vulkan swapchain image: "
                    + std::to_string(acquireResult));
        }
        imageAcquired = true;

        check(vkResetCommandBuffer(frame.commandBuffer, 0), "Could not reset desktop native Vulkan command buffer");

        VkCommandBufferBeginInfo beginInfo{};
        beginInfo.sType = VK_STRUCTURE_TYPE_COMMAND_BUFFER_BEGIN_INFO;
        check(vkBeginCommandBuffer(frame.commandBuffer, &beginInfo),
                "Could not begin desktop native Vulkan command buffer");
        ...
        logNativeError("Could not begin desktop native Vulkan frame", error);
        return 0;
    }
}

extern "C" void fdx_desktop_vulkan_end_frame(int64_t contextHandle) {
    Context* context = ptr<Context>(contextHandle);
    try {
        if (!context->frameStarted) {
            return;
        ...
        logNativeError("Could not end desktop native Vulkan frame", error);
    }
}

extern "C" void fdx_desktop_vulkan_read_pixels_rgba8(int64_t contextHandle, void* target, int32_t size) {
    Context* context = ptr<Context>(contextHandle);
    ReadbackBuffer readback{};
    try {
        if (!context->frameStarted) {
            throw std::runtime_error("Cannot read desktop native Vulkan pixels before beginFrame()");
        }
        if (!context->swapchainTransferSrcSupported) {
            throw std::runtime_error("desktop native Vulkan swapchain does not support transfer-source readback");
        }
        if (target == nullptr) {
            throw std::runtime_error("desktop native Vulkan readback requires a direct ByteBuffer");
        }
        ...
        copyReadbackToJava(context, &readback, target, size);
        ...
        logNativeError("Could not read desktop native Vulkan pixels", error);
    }
}

extern "C" void fdx_desktop_vulkan_clear(int64_t contextHandle, float red, float green, float blue, float alpha) {
    Context* context = ptr<Context>(contextHandle);
    try {
        ...
        logNativeError("Could not clear desktop native Vulkan frame", error);
    }
}

extern "C" int64_t fdx_desktop_vulkan_create_buffer(int64_t contextHandle, int32_t size, int32_t usage) {
    Context* context = ptr<Context>(contextHandle);
    try {
        return handle(createBuffer(context, static_cast<int>(size), static_cast<int>(usage)));
    } catch (const std::exception& error) {
        logNativeError("Could not create desktop native Vulkan buffer", error);
        return 0;
    }
}

extern "C" void fdx_desktop_vulkan_write_buffer(int64_t bufferHandle, void* data, int32_t size) {
    Buffer* buffer = ptr<Buffer>(bufferHandle);
    try {
        if (buffer == nullptr || buffer->context == nullptr) {
            throw std::runtime_error("desktop native Vulkan buffer is not valid");
        }
        if (size < 0 || static_cast<VkDeviceSize>(size) > buffer->size) {
            throw std::runtime_error("desktop native Vulkan buffer data is larger than the destination buffer");
        }
        if (data == nullptr) {
            throw std::runtime_error("desktop native Vulkan buffer writes require a direct ByteBuffer");
        }
        if (buffer->mapped == nullptr) {
            throw std::runtime_error("desktop native Vulkan buffer is not mapped");
        }
        std::memcpy(buffer->mapped, data, static_cast<size_t>(size));
    } catch (const std::exception& error) {
        logNativeError("Could not write desktop native Vulkan buffer", error);
    }
}
...
extern "C" int64_t fdx_desktop_vulkan_create_texture(int64_t contextHandle, int32_t width, int32_t height,
    ...
    try {
        ...
        logNativeError("Could not create desktop native Vulkan texture", error);
        return 0;
    }
}

extern "C" void fdx_desktop_vulkan_write_texture(int64_t textureHandle, void* data, int32_t size) {
    Texture* texture = ptr<Texture>(textureHandle);
    try {
        if (data == nullptr) {
            throw std::runtime_error("desktop native Vulkan texture writes require a direct ByteBuffer");
        }
        writeTextureData(texture, data, static_cast<int>(size));
    } catch (const std::exception& error) {
        logNativeError("Could not write desktop native Vulkan texture", error);
    }
}
...
extern "C" int64_t fdx_desktop_vulkan_create_shader_module(int64_t contextHandle,
    ...
    try {
        ...
        module->fragment = createShaderModule(context,
                wordsFromPointer(fragmentWords, fragmentWordCount, "Fragment SPIR-V words"));
        ...
        delete module;
        logNativeError("Could not create desktop native Vulkan shader module", error);
        return 0;
    }
}

extern "C" int64_t fdx_desktop_vulkan_create_render_pipeline(int64_t contextHandle, int64_t shaderModuleHandle,
    ...
    try {
        if (pipeline->sampledTextureCount < 0 || pipeline->sampledTextureCount > MAX_TEXTURE_DESCRIPTOR_SLOTS) {
            throw std::runtime_error("desktop native Vulkan sampled texture count exceeds the descriptor slot limit");
        }
        if (pipeline->uniformBufferEnabled
                && pipeline->sampledTextureCount != PBR_TEXTURE_DESCRIPTOR_COUNT) {
            throw std::runtime_error("desktop native Vulkan PBR uniform pipelines must declare five sampled textures");
        }
        ...
        std::vector<int> vertexStepModes = intsFromPointer(vertexStepModesData, vertexLayoutCount,
                "Vertex layout step modes", true);
        std::vector<int> attributeBindings = intsFromPointer(attributeBindingsData, attributeCount,
                "Vertex attribute bindings", true);
        std::vector<int> attributeLocations = intsFromPointer(attributeLocationsData, attributeCount,
                "Vertex attribute locations", true);
        std::vector<int> attributeFormats = intsFromPointer(attributeFormatsData, attributeCount,
                "Vertex attribute formats", true);
        std::vector<int> attributeOffsets = intsFromPointer(attributeOffsetsData, attributeCount,
                "Vertex attribute offsets", true);
        if (vertexStrides.size() != vertexStepModes.size()) {
            throw std::runtime_error("desktop native Vulkan vertex layout arrays must have the same length");
        }
        if (attributeBindings.size() != attributeLocations.size()
            ...
            throw std::runtime_error("desktop native Vulkan vertex attribute arrays must have the same length");
        }
        ...
        for (size_t i = 0; i < vertexStrides.size(); i++) {
            if (vertexStrides[i] <= 0) {
                throw std::runtime_error("desktop native Vulkan vertex layout stride must be greater than zero");
            }
        ...
        for (size_t i = 0; i < attributeLocations.size(); i++) {
            ...
            if (binding < 0 || binding >= static_cast<int>(vertexStrides.size())) {
                throw std::runtime_error("desktop native Vulkan vertex attribute binding is out of range");
            }
        ...
        check(vkCreatePipelineLayout(context->device, &layoutInfo, nullptr, &pipeline->layout),
                "Could not create desktop native Vulkan pipeline layout");
        ...
        check(vkCreateGraphicsPipelines(context->device, VK_NULL_HANDLE, 1, &pipelineInfo, nullptr,
                &pipeline->pipeline), "Could not create desktop native Vulkan graphics pipeline");
        ...
        delete pipeline;
        logNativeError("Could not create desktop native Vulkan render pipeline", error);
        return 0;
    }
}

extern "C" void fdx_desktop_vulkan_begin_render_pass(int64_t contextHandle, int32_t clear,
        float red, float green, float blue, float alpha, int32_t store, int32_t depthClear,
        float depthClearValue) {
    Context* context = ptr<Context>(contextHandle);
    try {
        if (!context->frameStarted) {
            throw std::runtime_error("Cannot begin desktop native Vulkan render pass outside a frame");
        }
        if (context->renderPassStarted) {
            throw std::runtime_error("desktop native Vulkan render pass is already active");
        }
        VkClearValue clearValues[2]{};
        clearValues[0].color.float32[0] = red;
        clearValues[0].color.float32[1] = green;
        ...
        VkRenderPassBeginInfo renderPassInfo{};
        renderPassInfo.sType = VK_STRUCTURE_TYPE_RENDER_PASS_BEGIN_INFO;
        renderPassInfo.renderPass = selectRenderPass(context, clear != 0, store != 0,
                depthClear != 0);
        renderPassInfo.framebuffer = context->framebuffers[context->imageIndex];
        renderPassInfo.renderArea.offset = {0, 0};
        renderPassInfo.renderArea.extent = context->extent;
        ...
        logNativeError("Could not begin desktop native Vulkan render pass", error);
    }
}

extern "C" void fdx_desktop_vulkan_set_pipeline(int64_t contextHandle, int64_t pipelineHandle) {
    Context* context = ptr<Context>(contextHandle);
    Pipeline* pipeline = ptr<Pipeline>(pipelineHandle);
    try {
        if (!context->renderPassStarted) {
            throw std::runtime_error("Cannot bind desktop native Vulkan pipeline outside a render pass");
        }
        vkCmdBindPipeline(currentFrame(context).commandBuffer, VK_PIPELINE_BIND_POINT_GRAPHICS,
                pipeline->pipeline);
    } catch (const std::exception& error) {
        logNativeError("Could not bind desktop native Vulkan pipeline", error);
    }
}

extern "C" void fdx_desktop_vulkan_set_vertex_buffer(int64_t contextHandle, int32_t slot, int64_t bufferHandle) {
    Context* context = ptr<Context>(contextHandle);
    Buffer* buffer = ptr<Buffer>(bufferHandle);
    try {
        if (!context->renderPassStarted) {
            throw std::runtime_error("Cannot bind desktop native Vulkan vertex buffer outside a render pass");
        }
        if (slot < 0) {
            throw std::runtime_error("desktop native Vulkan vertex buffer slot cannot be negative");
        }
        ...
        logNativeError("Could not bind desktop native Vulkan vertex buffer", error);
    }
}

extern "C" void fdx_desktop_vulkan_set_index_buffer(int64_t contextHandle, int64_t bufferHandle) {
    Context* context = ptr<Context>(contextHandle);
    Buffer* buffer = ptr<Buffer>(bufferHandle);
    try {
        if (!context->renderPassStarted) {
            throw std::runtime_error("Cannot bind desktop native Vulkan index buffer outside a render pass");
        }
        vkCmdBindIndexBuffer(currentFrame(context).commandBuffer, buffer->buffer, 0, VK_INDEX_TYPE_UINT16);
    } catch (const std::exception& error) {
        logNativeError("Could not bind desktop native Vulkan index buffer", error);
    }
}
...
extern "C" void fdx_desktop_vulkan_bind_textures(int64_t contextHandle, int64_t pipelineHandle,
    ...
    try {
        if (!context->renderPassStarted) {
            throw std::runtime_error("Cannot bind desktop native Vulkan textures outside a render pass");
        }
        if (textureHandles == nullptr) {
            throw std::runtime_error("desktop native Vulkan texture handles cannot be null");
        }
        ...
        logNativeError("Could not bind desktop native Vulkan textures", error);
    }
}
...
extern "C" void fdx_desktop_vulkan_bind_uniforms(int64_t contextHandle, int64_t pipelineHandle,
    ...
    try {
        if (!context->renderPassStarted) {
            throw std::runtime_error("Cannot bind desktop native Vulkan uniforms outside a render pass");
        }
        if (data == nullptr) {
            throw std::runtime_error("desktop native Vulkan uniform uploads require a direct ByteBuffer");
        }
        bindUniformDescriptor(context, pipeline, data, static_cast<int>(size));
    } catch (const std::exception& error) {
        logNativeError("Could not bind desktop native Vulkan uniforms", error);
    }
}

extern "C" void fdx_desktop_vulkan_draw(int64_t contextHandle, int32_t vertexCount,
        int32_t instanceCount, int32_t firstVertex, int32_t firstInstance) {
    Context* context = ptr<Context>(contextHandle);
    try {
        if (!context->renderPassStarted) {
            throw std::runtime_error("Cannot draw desktop native Vulkan geometry outside a render pass");
        }
        ...
        logNativeError("Could not draw desktop native Vulkan geometry", error);
    }
}

extern "C" void fdx_desktop_vulkan_draw_indexed(int64_t contextHandle, int32_t indexCount,
        int32_t instanceCount, int32_t firstIndex, int32_t baseVertex, int32_t firstInstance) {
    Context* context = ptr<Context>(contextHandle);
    try {
        if (!context->renderPassStarted) {
            throw std::runtime_error("Cannot draw indexed desktop native Vulkan geometry outside a render pass");
        }
        ...
        logNativeError("Could not draw indexed desktop native Vulkan geometry", error);
    }
}

extern "C" void fdx_desktop_vulkan_end_render_pass(int64_t contextHandle) {
    Context* context = ptr<Context>(contextHandle);
    try {
        if (!context->renderPassStarted) {
            return;
        ...
        logNativeError("Could not end desktop native Vulkan render pass", error);
    }
}

extern "C" int32_t fdx_desktop_vulkan_surface_format(int64_t contextHandle) {
    Context* context = ptr<Context>(contextHandle);
    return static_cast<int32_t>(context->surfaceFormat);
}

extern "C" void fdx_desktop_vulkan_destroy_shader_module(int64_t shaderModuleHandle) {
...
extern "C" void fdx_desktop_vulkan_destroy_buffer(int64_t bufferHandle) {
    Buffer* buffer = ptr<Buffer>(bufferHandle);
    if (buffer == nullptr) {
        return;
    }
...
extern "C" void fdx_desktop_vulkan_destroy_texture(int64_t textureHandle) {
    Texture* texture = ptr<Texture>(textureHandle);
    if (texture == nullptr) {
        return;
    }
