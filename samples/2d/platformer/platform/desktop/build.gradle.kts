...
base {
    archivesName.set("sample_platformer_desktop")
}
...
dependencies {
    ...
    if ((gradle.extensions.extraProperties.get("libfdxUsePublishedLibfdx") as Boolean)) {
        ...
        implementation("${libs.versions.libfdxGroup.get()}:audio_openal:$libfdxDependencyVersion")
...
libfdx {
    ...
    desktopJvm {
        mainClass.set("io.github.libfdx.samples.g2d.platformer.desktop.PlatformerDesktopLauncher")
        ...
        forwardSystemProperty("libfdx.sample.audio")
        target("gl") {
            displayName.set("GL")
            ...
            buildDescription.set("Builds the platformer desktop GL release jar.")
            runDescription.set("Runs the platformer desktop sample with GL.")
        }
        target("wgpu") {
            ...
            buildDescription.set("Builds the platformer desktop WGPU release jar.")
            runDescription.set("Runs the platformer desktop sample with WGPU.")
        }
        target("vulkan") {
            ...
            buildDescription.set("Builds the platformer desktop Vulkan release jar.")
            runDescription.set("Runs the platformer desktop sample with Vulkan.")
        }
        target("d3d12") {
            ...
            buildDescription.set("Builds the platformer desktop Direct3D 12 release jar.")
            runDescription.set("Runs the platformer desktop sample with Direct3D 12 on Windows.")
        }
    }
}
