...
import io.github.libfdx.ecs.tooling.EcsRenderPurpose;
...
final class SpriteMovementRuntime implements EcsProjectRuntime {
    ...
    private final SpriteMovementProject project;
    ...
    SpriteMovementRuntime(SpriteMovementProject project, long exitAfterFrames) {
    ...
    public void create(Fdx fdx) {
        ...
        world.addSystem(new PlayerControlSystem(new KeyboardMovementInput(fdx.input())));
        world.flushCommands();

        EcsSceneCodec sceneCodec = new EcsSceneCodec(project);
        String scene = readDefaultScene(fdx);
        sceneCodec.apply(world, sceneCodec.read(scene));
    ...
    public void render(EcsRenderContext context) {
        ...
        updateCamera();
        Camera renderCamera = context.camera() != null ? context.camera() : camera;
        ...
        RenderPass pass = context.frame().commandEncoder().beginRenderPass(renderPassDescriptor
                .colorAttachment(context.colorTarget())
        ...
        if (context.purpose() == EcsRenderPurpose.GAME
                && exitAfterFrames > 0L
    ...
    private void updateCamera() {
        cameraEntity = project.schema().cameras().activeCameraEntity(world);
        camera = cameraEntity != 0
                ? SpriteMovementSceneSchema.updateCamera(world, cameraEntity, width, height)
                : null;
    }

    private String readDefaultScene(Fdx fdx) {
        String path = project.defaultScene();
        try {
            return fdx.files().internal(path).readString(StandardCharsets.UTF_8).get();
        } catch (RuntimeException missingProjectPath) {
            int separator = path.lastIndexOf('/');
            String packagedPath = separator >= 0 ? path.substring(separator + 1) : path;
            try {
    ...
    static int wallTileCount(float worldSize) {
        return Math.max(1, (int) Math.ceil(Math.abs(worldSize) / WALL_TILE_SIZE));
    }

    static float toClipSize(float worldSize, float viewportSize) {
        return Math.abs(worldSize) * 2.0f / Math.max(MIN_VIEWPORT_SIZE, viewportSize);
    }

    static float toClipCenter(float worldPosition, float cameraPosition, float viewportSize) {
