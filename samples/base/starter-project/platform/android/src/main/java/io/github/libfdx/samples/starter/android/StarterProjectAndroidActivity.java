...
/**
 * Base Android activity for the Starter Project sample.
 *
 * @author xpenatan
 */
public abstract class StarterProjectAndroidActivity extends AndroidApplicationActivity {
    @Override
    protected AndroidApplicationConfig createApplicationConfig() {
        AndroidApplicationConfig config = new AndroidApplicationConfig()
                .title("libFDX Starter Project - " + graphicsDisplayName())
                .size(640, 480)
                .vSync(true)
                .foregroundFps(60)
    ...
    protected ApplicationListener createApplicationListener() {
        return new StarterProjectApplication();
    }
    ...
    protected String graphicsDisplayName() {
        ...
        if ("vulkan".equalsIgnoreCase(graphicsName())
                || "vk".equalsIgnoreCase(graphicsName())) {
            return "Vulkan JNI";
        }
        return "WGPU JNI";
    }
