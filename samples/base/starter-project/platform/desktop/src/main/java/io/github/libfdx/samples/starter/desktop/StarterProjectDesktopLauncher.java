package io.github.libfdx.samples.starter.desktop;
...
/**
 * Launches the Starter Project desktop entry point.
 *
 * @author xpenatan
 */
public final class StarterProjectDesktopLauncher {
    ...
    private StarterProjectDesktopLauncher() {
    }
    ...
    public static void main(String[] args) {
        ...
        DesktopApplicationConfig config = new DesktopApplicationConfig()
                .title("libFDX Starter Project - " + graphicsDisplayName(args, graphics))
                .size(640, 480)
                .maximized(maximized)
                .vSync(true)
                .foregroundFps(60)
        ...
        new DesktopApplicationBackend().start(
                config, new StarterProjectApplication(exitAfterFrames(args)));
    }
    ...
    private static long exitAfterFrames(String[] args) {
        String value = option(args, "--exit-after-frames=",
                System.getProperty("libfdx.sample.exitAfterFrames"));
        if (value != null) {
    ...
    private static Properties loadLaunchProperties() {
        ...
        try (InputStream stream = StarterProjectDesktopLauncher.class.getClassLoader()
